...
The parallel environment is initialized during the Startup phase (see [CLI and Lifecycle §5](../architecture/cli-and-lifecycle.md)). The ordering is critical — MPI must be initialized before OpenMP because MPI_Init_thread with `MPI_THREAD_MULTIPLE` must be the first MPI call.
