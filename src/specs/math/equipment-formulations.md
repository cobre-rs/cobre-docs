...
> **Stage-varying capacity**: The capacity limits $\bar{F}^+_l$ and $\bar{F}^-_l$ may vary by stage through exchange factors defined in `constraints/exchange_factors.json`.
...
The curtailment cost $c^{curt}_r$ is a **regularization** penalty (Category 3 in the [Penalty System](./penalty-system.md)), analogous to `spillage_cost` for hydros — curtailment discards available "free" energy.
...
- [Penalty System](./penalty-system.md) — penalty taxonomy, regularization vs. violation costs
