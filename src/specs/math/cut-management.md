...
Because each inner re-solve adds only a few rows from a retained basis, DCS and LP basis warm-start are complementary, not in tension. See [LP Warm-Start](lp-warm-start.md).
...
- [LP Warm-Start](lp-warm-start.md) — Basis warm-start that is complementary to DCS inner re-solves
...
- [Determinism Guarantees](determinism-guarantees.md) — Bit-identical results across thread and MPI-rank counts, including DCS resident-set seeding
