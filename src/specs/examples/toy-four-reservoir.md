...
[Toy Single-Reservoir Walkthrough](./toy-single-reservoir.md) to a
...
[LP Formulation](../math/lp-formulation.md), specialised to four
...
[PAR Inflow Model](../math/par-inflow-model.md) section 8 for the
...
[Stopping Rules](../math/stopping-rules.md) for the available
criteria.
...
- **FPHA production model**: nonlinear head-dependent productivity
  ...
  coefficient. See [Hydro Production Models](../math/hydro-production-models.md).
- **Spatial inflow correlation**: when a `correlation.json` file is
  ...
  [PAR Inflow Model](../math/par-inflow-model.md) section 8.
...
- **Cyclic policy graphs**: the four-stage horizon terminates without
  ...
  stage. See [Horizon Modes](../math/horizon-modes.md).
- **Multi-resolution coupling**: weekly intra-stage blocks embedded
  ...
  See [Multi-Resolution Studies](../math/multi-resolution-studies.md).
...
- [Cut Management](../math/cut-management.md) — Dual extraction, per-opening intercepts, single-cut aggregation; sign convention $\pi^v = \partial Q/\partial \hat v$
- [Risk Measures](../math/risk-measures.md) — CVaR definition, EAVaR convex combination, risk-adjusted aggregation weights
- [Horizon Modes](../math/horizon-modes.md) — Finite vs cyclic policy graphs and the season-indexed cut pool
