...
This spec defines the `RiskMeasure` abstraction -- the enum-based trait through which the SDDP backward pass aggregates per-opening outcomes into cut coefficients and evaluates risk-adjusted costs for convergence monitoring. The risk measure is the mechanism by which scenario probabilities $p(\omega)$ are replaced with risk-adjusted weights $\mu^*_\omega$ during cut aggregation, as defined in [Risk Measures §7](../math/risk-measures.md). Because the risk measure can vary per stage (configured via the `risk_measure` field in `stages.json`), the abstraction uses enum dispatch rather than compile-time monomorphization, following the analysis in [Extension Points §7](./extension-points.md).
...
The risk measure is modeled as a flat enum with two variants, matching the two risk measure types supported by Cobre ([Extension Points §2.1](./extension-points.md)):
...
/// loading (see Extension Points §6). The enum is matched at each backward
...
pub enum RiskMeasure {
    ...
    /// [Cut Management §3](../math/cut-management.md).
    Expectation,

    /// Convex combination of expectation and CVaR:
    /// ρ^{λ,α}[Z] = (1 - λ) E[Z] + λ · CVaR_α[Z]
    ///
    /// See [Risk Measures §3](../math/risk-measures.md) for the definition
    /// and [Risk Measures §7](../math/risk-measures.md) for the weight
...
/// from the LP dual variables as described in [Cut Management §2].
...
| For `Expectation`: result equals probability-weighted sum                                  | $\bar{\alpha} = \sum_\omega p(\omega) \cdot \hat{\alpha}(\omega)$, $\bar{\pi}_h = \sum_\omega p(\omega) \cdot \pi_h(\omega)$ -- identical to [Cut Management §3](../math/cut-management.md)      |
| For `CVaR`: result equals risk-weighted sum using $\mu^*_\omega$                           | $\bar{\alpha} = \sum_\omega \mu^*_\omega \cdot \hat{\alpha}(\omega)$, $\bar{\pi}_h = \sum_\omega \mu^*_\omega \cdot \pi_h(\omega)$ -- as defined in [Risk Measures §7](../math/risk-measures.md) |
| Risk-adjusted weights $\mu^*_\omega$ satisfy $\sum_\omega \mu^*_\omega = 1$                | Weights form a valid probability distribution                                                                                                                                                    |
| Risk-adjusted weights satisfy $0 \leq \mu^*_\omega \leq \bar{\mu}_\omega$ for all $\omega$ | Each weight is bounded by the per-scenario upper bound from [Risk Measures §7](../math/risk-measures.md)                                                                                         |

**Behavioral contract for weight computation:** When the variant is `CVaR { alpha, lambda }`, the risk-adjusted weights $\mu^*_\omega$ must be the optimal solution to the dual LP from [Risk Measures §4.2](../math/risk-measures.md). The per-scenario upper bound is $\bar{\mu}_\omega = (1 - \lambda) p_\omega + \lambda p_\omega / \alpha$, and the optimal weights place maximum mass on the highest-cost scenarios. The math spec establishes that a sorting-based greedy allocation produces the same result as the dual LP ([Risk Measures §7, equivalence note](../math/risk-measures.md)). The implementation may use either approach.

**Infallibility:** This method does not return `Result`. All inputs are validated at configuration load time ([Extension Points §6](./extension-points.md)), and the LP solver guarantees finite objective values when relatively complete recourse holds ([Cut Management §4](../math/cut-management.md)).
...
| For `CVaR`: result equals $(1 - \lambda) \mathbb{E}[Z] + \lambda \cdot \text{CVaR}_\alpha[Z]$ | Convex combination as defined in [Risk Measures §3](../math/risk-measures.md) |
| `Expectation` result $\leq$ `CVaR` result (when $\alpha < 1$)                                 | CVaR places more weight on worst outcomes                                     |

**Bound validity warning:** When CVaR is active on any stage, the first-stage LP objective is a convergence indicator only -- it is NOT a valid lower bound on the true risk-averse optimal cost. See [Risk Measures §10](../math/risk-measures.md) for the full explanation.
...
The `RiskMeasureConfig` enum represents the deserialized form of the `risk_measure` field in `stages.json` ([Input Scenarios §1.7](../data-model/input-scenarios.md)). It maps directly to the JSON schema:
...
**Conversion:** `RiskMeasureConfig` is validated and converted to `RiskMeasure` during configuration loading (step 3 of the variant selection pipeline in [Extension Points §6](./extension-points.md)). The validation rules are specified in SS5.

### 3.2 Per-Stage Risk Measure Array

Because the risk measure can vary per stage ([Risk Measures §8](../math/risk-measures.md)), the training loop holds a per-stage array:
...
**Why not trait objects:** The variant set is closed (Expectation and CVaR only, with no additional variants planned -- see [Extension Points §2.5](./extension-points.md)). Trait objects add indirection cost without the extensibility benefit. The enum approach is consistent with the dispatch analysis in [Extension Points §7](./extension-points.md).
...
The following validation rules apply to `RiskMeasureConfig` during configuration loading. These reproduce rules R1-R3 from [Extension Points §2.3](./extension-points.md):
...
Validation is performed once during the variant selection pipeline ([Extension Points §6](./extension-points.md), step 5). After validation, the `RiskMeasure` enum values are guaranteed to satisfy these constraints for the entire training run. This is why `aggregate_cut` and `evaluate_risk` are infallible -- they operate on validated inputs.
...
- [Risk Measures](../math/risk-measures.md) -- Mathematical definitions: CVaR (§2), convex combination (§3), dual representation (§4), risk-averse subgradient theorem (§5), cut generation with risk measures (§7), per-stage profiles (§8), lower bound invalidity (§10)
- [Cut Management](../math/cut-management.md) -- Dual extraction (§2), single-cut aggregation (§3) that risk-neutral weights reproduce, cut validity conditions (§4)
- [Cut Management Implementation](./cut-management-impl.md) -- FCF runtime structure (§1), deterministic slot assignment (§1.2), cross-rank cut synchronization (§4)
- [Extension Points](./extension-points.md) -- Risk measure variant table (§2.1), configuration mapping (§2.2), validation rules R1-R3 (§2.3), behavioral contract (§2.4), dispatch mechanism analysis (§7), variant selection pipeline (§6)
- [Training Loop](./training-loop.md) -- Backward pass (§6) where `aggregate_cut` is invoked; behavioral contracts for abstraction points (§3)
