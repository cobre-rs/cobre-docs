...
Two Benders cuts are provided for `add_cut_rows` tests. Each cut has the form $x_1 \geq \alpha + \beta \cdot x_0$, which in LP row form is $-\beta \cdot x_0 + x_1 \geq \alpha$ (coefficients on the theta column and state column).
...
For `patch_row_bounds` tests, the state-fixing constraint (Row 0) RHS is changed from 6.0 to 4.0, simulating a different incoming state $x_{t-1} = 4$.
...
### SS1.5 add_cut_rows Conformance

| Test Name                                   | Input Scenario                                                                                               | Expected Observable Behavior                                                                                           | Variant |
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- | ------- |
| `test_solver_highs_add_cut_rows_tightens`   | Load shared fixture. Add both cuts from SS1.2 via `add_cut_rows`. Solve.                                     | Objective = 162.0. Primal: $x_0 = 6.0$, $x_1 = 62.0$, $x_2 = 2.0$. The cuts tighten the objective from 100.0 to 162.0. | HiGHS   |
| `test_solver_clp_add_cut_rows_tightens`     | Same as above.                                                                                               | Objective = 162.0. Primal: $x_0 = 6.0$, $x_1 = 62.0$, $x_2 = 2.0$.                                                     | CLP     |
| `test_solver_highs_add_cut_rows_single_cut` | Load shared fixture. Add only Cut 1 ($x_1 \geq 20 + 5 x_0$) via `add_cut_rows` with a 1-row CutBatch. Solve. | Objective = 150.0 (= 0 + 50 + 100). Primal: $x_0 = 6.0$, $x_1 = 50.0$, $x_2 = 2.0$.                                    | HiGHS   |
| `test_solver_clp_add_cut_rows_single_cut`   | Same as above.                                                                                               | Objective = 150.0. Primal: $x_0 = 6.0$, $x_1 = 50.0$, $x_2 = 2.0$.                                                     | CLP     |

### SS1.6 patch_row_bounds Conformance

| Test Name                                         | Input Scenario                                                                                            | Expected Observable Behavior                                                   | Variant |
| ------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ | ------- |
| `test_solver_highs_patch_row_bounds_state_change` | Load shared fixture. Add both cuts. Patch Row 0 RHS from 6.0 to 4.0 (both lower and upper bounds). Solve. | Objective = 368.0. Primal: $x_0 = 4.0$, $x_1 = 68.0$, $x_2 = 6.0$ (per SS1.3). | HiGHS   |
| `test_solver_clp_patch_row_bounds_state_change`   | Same as above.                                                                                            | Objective = 368.0. Primal: $x_0 = 4.0$, $x_1 = 68.0$, $x_2 = 6.0$.             | CLP     |

### SS1.6a patch_col_bounds Conformance

**Test naming convention:** `test_solver_{variant}_patch_col_bounds_{scenario}` where `{variant}` is `highs` or `clp` and `{scenario}` describes the column-bound patching test case.

These tests verify the `patch_col_bounds` method contract from [Solver Interface Trait SS2.3a](./solver-interface-trait.md). Each test starts from the shared LP fixture (SS1.1) and applies column-bound patches via `(col_index, new_lower, new_upper)` triples. The method is infallible (panics on invalid column index) and preserves the solver basis.

| Test Name                                     | Input Scenario                                                                                                                                                                                                                                                                                                           | Expected Observable Behavior                                                                                                                                                                                                                                                                                                                                                                        | Variant |
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| `test_solver_highs_patch_col_bounds_basic`    | Load shared fixture. Add both cuts (SS1.2). Patch column 2 ($x_2$, thermal gen) bounds from $[0, 8]$ to $[0, 3]$ via `patch_col_bounds(&[(2, 0.0, 3.0)])`. Solve.                                                                                                                                                        | Solve returns `Ok`. Objective = 162.0. Primal: $x_0 = 6.0$, $x_1 = 62.0$, $x_2 = 2.0$. The tighter upper bound ($x_2 \leq 3$) does not bind because the optimal $x_2 = 2.0 < 3.0$. Solution is unchanged from the unpatched case (SS1.2). Non-patched column bounds ($x_0$, $x_1$) are unchanged. Row bounds are unchanged.                                                                         | HiGHS   |
| `test_solver_clp_patch_col_bounds_basic`      | Same as above.                                                                                                                                                                                                                                                                                                           | Same: objective = 162.0, primal = $(6.0, 62.0, 2.0)$. Column-bound patch does not bind.                                                                                                                                                                                                                                                                                                             | CLP     |
| `test_solver_highs_patch_col_bounds_tightens` | Load shared fixture (no cuts). Patch column 1 ($x_1$, future cost theta) bounds from $[0, +\infty)$ to $[10, +\infty)$ via `patch_col_bounds(&[(1, 10.0, f64::INFINITY)])`. Solve.                                                                                                                                       | Solve returns `Ok`. Objective = 110.0. Primal: $x_0 = 6.0$, $x_1 = 10.0$, $x_2 = 2.0$. Without the patch, $x_1 = 0$ (at original lower bound 0). With the tighter lower bound $x_1 \geq 10$, the theta variable is pushed to its new lower bound $x_1 = 10.0$. Objective: $0(6) + 1(10) + 50(2) = 110.0$, up from 100.0. Non-patched column bounds ($x_0$, $x_2$) and all row bounds are unchanged. | HiGHS   |
| `test_solver_clp_patch_col_bounds_tightens`   | Same as above.                                                                                                                                                                                                                                                                                                           | Same: objective = 110.0, primal = $(6.0, 10.0, 2.0)$. Tighter lower bound on $x_1$ is active.                                                                                                                                                                                                                                                                                                       | CLP     |
| `test_solver_highs_patch_col_bounds_repatch`  | Load shared fixture (no cuts). Solve (objective = 100.0, $x_1 = 0.0$). Patch column 1 bounds to $[10, +\infty)$ via `patch_col_bounds(&[(1, 10.0, f64::INFINITY)])`. Solve (objective = 110.0, $x_1 = 10.0$). Re-patch column 1 bounds back to $[0, +\infty)$ via `patch_col_bounds(&[(1, 0.0, f64::INFINITY)])`. Solve. | Third solve returns `Ok`. Objective = 100.0. Primal: $x_0 = 6.0$, $x_1 = 0.0$, $x_2 = 2.0$. Re-patching column bounds restores the original feasible region. The solver correctly applies each successive column-bound patch before the next solve. Basis is preserved across patches.                                                                                                              | HiGHS   |
| `test_solver_clp_patch_col_bounds_repatch`    | Same as above.                                                                                                                                                                                                                                                                                                           | Same: three solves produce objectives 100.0, 110.0, 100.0. Column bounds are correctly re-applied at each step.                                                                                                                                                                                                                                                                                     | CLP     |
...
| `test_solver_highs_get_basis_with_cuts`  | Load shared fixture. Add both cuts (4 total rows). Solve. Call `get_basis`.                                                            | `basis.col_status.len() == 3`. `basis.row_status.len() == 4` (2 structural + 2 cut rows). Basis includes cut row statuses per [Solver Abstraction SS9](./solver-abstraction.md).                         | HiGHS   |
