...
#### Future delivery on a non-anticipated thermal

**When it occurs:** `initial_conditions.json`'s `future_anticipated_deliveries`
entries declare an in-study decided delivery commitment for an anticipated
thermal, keyed by `thermal_id`. Layer 5 (semantic validation) rejects any entry
whose `thermal_id` does not resolve to a thermal with `anticipated_config:
Some(_)` — either the thermal does not exist, or it exists but is not
configured as anticipated. This is the delivery-side mirror of the same check
performed on `past_anticipated_commitments`.

**Fields:**

| Field         | Type     | Description                                                                                                                                                                                                                     |
| ------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `description` | `String` | Contains a `[BusinessRuleViolation] initial_conditions.json (thermal_id=<id>, delivery_start=<date>): Thermal <id>: referenced in future_anticipated_deliveries but is not an anticipated thermal (anticipated_config is None or thermal does not exist)` line |

**Example:**

```
constraint violation: [BusinessRuleViolation] initial_conditions.json (thermal_id=12, delivery_start=2026-09-05): Thermal 12: referenced in future_anticipated_deliveries but is not an anticipated thermal (anticipated_config is None or thermal does not exist)
```
...
`future_anticipated_deliveries` entry. See
...
**When it occurs:** Layer 5 resolves every `future_anticipated_deliveries`
window against `post_study_stages.json`, the post-horizon boundary calendar a
delivery matures into. Several distinct failures are collected under this one
check, in the order they are evaluated:

- No `post_study_stages.json` at all, while `future_anticipated_deliveries` is
  non-empty — every delivery is unanchored.
...
- The post-study stages are not date-contiguous (a gap or overlap between
  consecutive stages).
- A delivery window reaches outside the declared post-study horizon.
- A delivery window is not covered exactly (coverage `1.0`) by the post-study
  calendar — it does not align to whole post-study stage boundaries.
- A delivery window spans more than one post-study stage (multi-stage delivery
  is not supported).
- No `thermal_bounds` cell exists for the `(thermal_id, post_study_stage_index)`
  a delivery resolves into.
- The delivery's committed `[min_mw, max_mw]` interval and the `thermal_bounds`
  cell's capability `[min_mw, max_mw]` do not intersect.
...
```
constraint violation: [BusinessRuleViolation] post_study_stages.json (thermal_id=12, delivery_start=2026-09-05): Thermal 12: future_anticipated_deliveries window [2026-09-05, 2026-09-12) has no post_study_stages.json to resolve against; declare a post-study boundary calendar covering it.
```
...
```
constraint violation: [BusinessRuleViolation] post_study_stages.json (thermal_id=12, delivery_start=2026-10-01): Thermal 12: future_anticipated_deliveries window [2026-10-01, 2026-10-10) reaches outside the post-study horizon [2026-09-05, 2026-10-05); every delivery must fall within the declared post-study stages.
```

```
constraint violation: [BusinessRuleViolation] post_study_stages.json (thermal_id=12, delivery_start=2026-09-10): Thermal 12: future_anticipated_deliveries window [2026-09-10, 2026-09-20) is not covered exactly (1.0) by the post-study stages; align the window to whole post-study stage boundaries.
```

```
constraint violation: [BusinessRuleViolation] post_study_stages.json (thermal_id=12, delivery_start=2026-09-05): Thermal 12: future_anticipated_deliveries window [2026-09-05, 2026-11-05) spans 2 post-study stages; a delivery window must resolve to exactly one post-study stage (multi-stage delivery is not supported).
```

```
constraint violation: [BusinessRuleViolation] post_study_stages.json (thermal_id=12, delivery_start=2026-09-05): Thermal 12: future_anticipated_deliveries window [2026-09-05, 2026-10-05) delivers into post-study stage index 0, but post_study_stages.json has no thermal_bounds entry for (thermal_id 12, post_study_stage_index 0).
```

```
constraint violation: [BusinessRuleViolation] post_study_stages.json (thermal_id=12, delivery_start=2026-09-05): Thermal 12: future_anticipated_deliveries commitment interval [300.0, 350.0] and post-study stage index 0 capability [0.0, 250.0] do not intersect; the committed amount is unsatisfiable within capability.
```

**Resolution:** Declare `post_study_stages.json` whenever
`future_anticipated_deliveries` is non-empty, with its first stage starting
exactly at the study horizon end and every stage date-contiguous with the next.
Align each delivery window to whole post-study stage boundaries and keep it
within a single stage. Add a `thermal_bounds` cell for every
`(thermal_id, post_study_stage_index)` a delivery resolves into, with a
capability interval that intersects the delivery's committed interval. See
...
windows must tile its leading `K` (calendar-derived) delivery stages exactly —
every leading stage covered at fraction `1.0`, and no window reaching a stage
at or beyond `K`. Layer 5 (semantic validation) reports a gap (an uncovered
leading stage) and an over-reach (a window extending past the leading `K`
stages) as separate diagnostics; either can fire independently.
...
```
constraint violation: [BusinessRuleViolation] initial_conditions.json (thermals[id=7].anticipated_config): Thermal 7: past_anticipated_commitments cover study stage id(s) [3] beyond the leading 2 calendar-derived delivery stage(s); commitment windows must not extend past the pre-study delivery horizon.
```

**Resolution:** Write a commitment window for every one of the plant's leading
`K` delivery stages (a committed `0` MW is a legitimate, explicit window — not
an omission), and do not extend any window past stage `K - 1`. `K` is
...
**When it occurs:** `constraints/hydro_bounds.parquet`'s row-level parser
...
a top-level `LoadError::SchemaError` (`schema error in {path}, field {field}:
{message}`, per the [`SchemaError`](#schemaerror) `LoadError` variant above).
...
**See also:** the post-study boundary / future-delivery cluster is also
reported this way — see
[Future delivery on a non-anticipated thermal](#future-delivery-on-a-non-anticipated-thermal),
