...
are mandatory keys with no default when absent. A root-level `training.forward_passes` key
...
A `manifest.bin` whose `format_version` is not exactly `2` fails earlier, as a
checkpoint read error, not as `PolicyIncompatible` (see
[Policy Management](/running/policy-management/)).
...
The `PolicyIncompatible` display format shown for the variant is
...
current system: it fails one of the policy-load checks listed under
[`PolicyIncompatible`](#policyincompatible). The policy was produced by a run with a
different system configuration. This `ErrorKind` is the validation diagnostic
counterpart to the `PolicyIncompatible` load error.
