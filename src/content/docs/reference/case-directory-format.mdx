...
Input Parquet files must be uncompressed or zstd-compressed; Snappy, gzip and LZ4
are rejected at load as a `ParseError`
(`Parquet error: Disabled feature at compile time: snap` for Snappy). pyarrow
`pq.write_table` writes Snappy by default, so pass `compression="zstd"`; polars
`write_parquet` already defaults to zstd.
...
| `exports`                | object | all disabled | Output file selection flags                                         |
...
| `modeling.cost_scale_factor`            | number | `1_000_000.0` | Divisor on non-θ objective coefficients; objective conditioning only (see [Configuration](/running/configuration/#cost_scale_factor)) |
...
| `training.stopping_rules`  | array           | **required**                        | List of stopping-rule entries; a missing key is a hard load error. An empty array `[]` is treated as a single `iteration_limit` rule of 100 iterations |
...
| `training.parallelism`     | object or null  | `{backward_scheduler: by_scenario}` | Backward-pass scheduler selection; see [Configuration](/running/configuration/#parallelism)                                |
| `training.solver`          | object or null  | `null`                              | LP solver retry policy and optional per-phase solver profiles; see [Configuration](/running/configuration/#trainingsolver) |
...
When absent, all classes default to `"in_sample"` (PAR-based noise generation).

| Field                                       | Type            | Default                     | Description                                                                                            |
| ------------------------------------------- | --------------- | --------------------------- | ------------------------------------------------------------------------------------------------------ |
| `training.scenario_source.seed`             | integer or null | `null`                      | Forward-pass seed; **required** when any class uses `"out_of_sample"`, `"historical"`, or `"external"` |
...
| `training.scenario_source.load.scheme`      | string          | `"in_sample"`               | Load sampling scheme: `"in_sample"`, `"external"`, or `"out_of_sample"` (no replay scheme; see the admission rules) |
| `training.scenario_source.ncs.scheme`       | string          | `"in_sample"`               | NCS sampling scheme: `"in_sample"`, `"external"`, or `"out_of_sample"` (no replay scheme; see the admission rules)  |
...
| `training.scenario_source.openings`         | object          | `{"source": "generated"}`   | Backward opening-tree source: `{"source": "generated"}` builds the tree internally; `{"source": "file"}` reads `scenarios/noise_openings.parquet` (see [`scenarios/noise_openings.parquet`](#scenariosnoise_openingsparquet)). Training only |

**Admission rules.** Both `training.scenario_source` and `simulation.scenario_source`
are checked at load; each violation is a `SchemaViolation` on `config.json` naming the
field (`<section>` is `training` or `simulation`):

- `historical` is inflow-only — `field <section>.scenario_source.load.scheme` (or
  `.ncs.scheme`): `historical scheme is only valid for the inflow class`.
- `seed` is required whenever any class is not `"in_sample"` —
  `field <section>.scenario_source.seed`:
  `seed is required when any class uses out_of_sample, historical, or external scheme`.
- `historical_years` needs a class on `"historical"` —
  `field <section>.scenario_source.historical_years`:
  `historical_years is specified but no class uses the 'historical' scheme`.
- A `historical_years` range must satisfy `from <= to` —
  `field <section>.scenario_source.historical_years`: for example `range 'from' (2010) must be <= 'to' (2000)`.
- `openings` is training-only — `field simulation.scenario_source.openings`:
  `openings is only valid under training.scenario_source, not simulation.scenario_source`.

With `openings = {"source": "file"}`, a missing `scenarios/noise_openings.parquet` fails
the load (`openings source is 'file' but the conventional scenarios/noise_openings.parquet is absent`),
and the declaration is rejected together with a declared `nodes[]` policy graph or under
`training.selection.method = "enumerated"`.
...
| `gap`             | `tolerance: number` and/or `relative_tolerance: number` (at least one required) | The optimality gap between the lower and upper bounds falls below the tolerance |

:::note[`stopping_rules` need not include `iteration_limit`]
Earlier documentation said the rule set must include an `iteration_limit` rule.
The binary requires the `training.stopping_rules` key, but accepts any non-empty
set without an `iteration_limit` rule. Training still never runs past an
iteration budget: the largest `iteration_limit` in the set, or 100 iterations
when the set has none. An empty array `[]` is treated as a single
`iteration_limit` rule of 100 iterations. These pages follow the binary.
:::
...
| `boundary`                          | object or null | `null`       | Terminal boundary cut config: `path` (string) + `strict` (bool, default false) |
...
| `simulation.scenario_source`                  | object or null  | `null`        | Per-class sampling scheme for the simulation pass; when absent, the simulation reuses `training.scenario_source` as a whole. Same admission rules as training; `openings` is rejected here |
| `simulation.scenario_source.seed`             | integer or null | `null`        | Forward-pass seed; **required** when any class uses `"out_of_sample"`, `"historical"`, or `"external"` |
...
| `simulation.scenario_source.load.scheme`      | string          | `"in_sample"` | Load sampling scheme: `"in_sample"`, `"external"`, or `"out_of_sample"` (no replay scheme; see the admission rules) |
| `simulation.scenario_source.ncs.scheme`       | string          | `"in_sample"` | NCS sampling scheme: `"in_sample"`, `"external"`, or `"out_of_sample"` (no replay scheme; see the admission rules)  |
...
| `fpha_deviation_points` | boolean | `false` | Export the per-grid-point computed-FPHA fit-deviation table to `output/hydro_models/fpha_deviation_points.parquet`. |
...
All four sections are required. Every scalar cost must be strictly positive
(`> 0.0`), with one exception: the hydro `turbined_cost` regularization cost is
validated `>= 0.0` (a `0.0` turbined cost — no turbine-flow penalty — is valid).
...
| `blocks`          | Yes      | Array of load blocks (`id`, `name`, `hours`); must be non-empty, and every `hours` must be finite and `> 0` (an empty array is rejected as `InvalidValue`, a non-positive `hours` at parse time as `SchemaViolation`) |
...
Initial reservoir storage, pre-study releases already in transit on declared
travel-time arcs, and recent observations at the start of the study.
...
(the classic pre-horizon ring seed) or past it (the DECOMP _já-comandada_ case,
...
stages inside the study, and any post-horizon stages of a _já-comandada_
...
[system elements §4](/math/system-elements#anticipated-thermal-plants) for
...
:::note[Post-horizon deliveries have two declaration surfaces]
A commitment **decided in-study** whose delivery matures after the horizon is
no longer declared here. It is bounded and costed **solely** by a
`thermal_bounds[]` cell in [`post_study_stages.json`](#post_study_stagesjson)
below — the study itself decides the MW, so only its price and capability
need declaring. The removed `future_anticipated_deliveries[]` field carried
that role; an `initial_conditions.json` still declaring it is rejected by the
file's deny-unknown-fields contract. A commitment **decided before the study**
that delivers post-horizon (_já-comandada_) is instead a
`past_anticipated_commitments` window extending past the horizon end (above).
See [Post-Study Boundary](/math/post-study-boundary) for the boundary
formulation both feed.
:::
...
[Post-Study Boundary](/math/post-study-boundary) for the boundary formulation
this file feeds.
...
```json
{
  "$schema": "https://docs.cobre-rs.dev/schemas/post_study_stages.schema.json",
  "stages": [
...
The vendored JSON Schema is at
[`/schemas/post_study_stages.schema.json`](/schemas/post_study_stages.schema.json) —
see [JSON Schemas](/reference/json-schemas) for the full schema-to-file table
...
| `hydros[].specific_productivity_mw_per_m3s_per_m` | No       | Specific productivity ρ_esp [MW/(m³/s)/m]. Required for FPHA hydros that derive ρ_eq from VHA geometry; on any hydro with `system/hydro_geometry.parquet` rows it also feeds the useful-range mean evaluator (per-stage overrides in `system/hydro_energy_productivity.parquet`). |
...
| Field               | Required | Description                                                                  |
| ------------------- | -------- | ------------------------------------------------------------------------------ |
| `id`                | Yes      | Group identifier: unique **within the plant** (not globally)   |
...
[`constraints/hydro_unit_group_bounds.parquet`](#constraintshydro_unit_group_boundsparquet).
Per-cell simulation output is
[`simulation/hydro_bus_generation/`](/reference/output-format/#simulationhydro_bus_generation);
...
`cobre validate` (see
[`GenericConstraintValidationError`](/reference/error-codes/#genericconstraintvalidationerror)).
...
**`fpha_plane_reduction` (optional top-level block).** A sibling of the
...
rejected, so a tolerance field that belongs to the _other_ method is rejected.

| Field (`"method": "angle"`) | Required | Description                                              |
| --------------------------- | -------- | -------------------------------------------------------- |
| `method`                    | Yes      | The literal string `"angle"`.                            |
| `tolerance_deg`             | Yes      | Merge tolerance in degrees; finite and in `[0.0, 90.0]`. |

| Field (`"method": "distance"`) | Required | Description                                     |
| ------------------------------ | -------- | ----------------------------------------------- |
| `method`                       | Yes      | The literal string `"distance"`.                |
| `tolerance_pct`                | Yes      | Distance tolerance (percentage); finite, `>= 0.0`. |
| `n_samples`                    | Yes      | Sample count; integer `>= 1`.                   |
...
| `spillage_discretization_points` | No       | `5`            | Validated (`>= 2`); does not add a spillage axis to the fit — accepted for input round-tripping (see [Hydro Production Models → Configure](/math/hydro-production-models#implementation-in-cobre)) |
| `max_planes_per_hydro`           | No       | `10`           | Validated (`>= 1`); does not currently trim the fitted plane set (see [Hydro Production Models → Configure](/math/hydro-production-models#implementation-in-cobre)) |
...
all stages of that hydro. One row per hyperplane; at least 1 plane is required
...
- `equivalent_productivity_mw_per_m3s` — replaces ρ_eq outright in **both**
  energy-conversion evaluators (the reference-point value and the useful-range
  mean value), and therefore the `equivalent_productivity` /
  `integrated_equivalent_productivity` computed tags and the
  `equivalent_productivity_mw_per_m3s` /
  `integrated_equivalent_productivity_mw_per_m3s` simulation output columns.
- `specific_productivity_mw_per_m3s_per_m` — the ρ_esp that multiplies the net
  head in **both** evaluators' head-derived ρ_eq (the reference-point value of an
  FPHA plant; the useful-range mean value of any plant with VHA geometry), and
  the `specific_productivity` computed tag. It is resolved per
  `(hydro, stage)` as stage-specific row → `stage_id = NULL` row → the entity's
  `specific_productivity_mw_per_m3s_per_m` in `system/hydros.json`.
- `reference_outflow_m3s` — reaches the `reference_turbine` computed tag
  **only**. It does not change the net head: both evaluators always evaluate the
  head at the plant's `max_turbined_m3s`.
...
| `specific_productivity_mw_per_m3s_per_m` | DOUBLE       | yes      | ρ_esp override [MW/(m³/s)/m]; finite and >= 0.0 (`0.0` marks a planned-outage stage)                                           |
...
[`initial_conditions.json`](#initial_conditionsjson) above) — as well as the
raw observations Cobre estimates `inflow_seasonal_stats`/`inflow_ar_coefficients`
from when those files are absent.

### `scenarios/inflow_seasonal_stats.parquet`

PAR(p) model seasonal statistics for each (hydro plant, stage) pair. Not
...
Autoregressive coefficients for the PAR(p) inflow model. The model's
innovation scale is not an input: Cobre derives it at load from the
coefficients below via the periodic-ACF closure — see
...
Columns beyond this schema are ignored — a file carrying an extra column (for
example a stored innovation-scale ratio produced by another tool) loads with
that column ignored outright; the derived value always wins.
...
[`config.json`](#configjson)). This enables cross-tool comparison, sensitivity
...
- **Dimension mismatch** — the number of distinct `entity_index` values must
  equal the noise-vector length defined above.
- **Stage count mismatch** — the distinct `stage_id` values must be the
  declared study stages.
- **Missing opening indices** — for each stage, every opening index from 0 to
  that stage's `num_openings − 1` must be present for every entity. Gaps are
  not permitted; partial-stage override is not supported.

The total row count must equal the sum over study stages of
`num_openings × dimension`.

See [Scenario Generation](/math/scenario-generation) for the opening tree and
[Configuration](/running/configuration/#scenario_source) for the `openings`
field.
...
> max_spillage_m3s` is rejected. See [Error Codes](/reference/error-codes) for
...
[`system/hydros.json`](#systemhydrosjson) above). A row overrides one of a
group's four declared bounds for one `(hydro_id, hydro_unit_group_id,
stage_id)`, optionally narrowed to a single block; a group with no override
...
entity-level value need rows. Absent rows keep the entity's
...
User-defined linear constraints, added to every stage's LP alongside the
built-in constraint set. The file is optional; when absent, no generic
...
```json
{
  ...
  "expressions": [
    { "name": "total_hydro", "expression": "hydro_generation(1) + hydro_generation(2)" }
  ]
}
```

Alongside `constraints`, the file accepts an optional top-level `expressions`
array — named linear expressions (`{ "name", "expression" }`) shared by every
constraint and referenced by name from any `expression`. It is absent by
default. See
[Generic Constraints](/reference/generic-constraints/) for the named-expression
...
[Generic Constraints: Interval and shape derivation](/reference/generic-constraints/#interval-and-shape-derivation)
...
[`hydros[].unit_groups[]`](#systemhydrosjson) above): `hydro_turbined(5,
bus=2)` and `hydro_generation(5, bus=2)` resolve to the LP column for hydro
...
[`system/thermals.json`](#systemthermalsjson) above for `anticipated_config`.

`hydro_useful_volume_initial(id[, block])` and
`hydro_useful_volume_final(id[, block])` address the same LP column and accept
the same block argument as `hydro_storage_initial`/`hydro_storage_final`, but
read as storage above the plant's physical minimum
(`reservoir.min_storage_hm3`); see
[Generic Constraints](/reference/generic-constraints/#variable-catalog) for the
bound fold that realises them.
...
**is** the activation grid: a constraint is active at a given `(stage[,
block])` if and only if a row exists for it here, regardless of whether that
...
constraint whose only bound endpoints come from an inline affine remainder but
which has zero rows here fails referential validation (`InvalidReference` — the
reference would apply to nothing): the activation grid decides _where_ such a
constraint applies, even when it supplies none of the bound _value_. A plain
constraint with no affine remainder and no rows is not an error — it simply
applies to nothing and is silently inert. See
[Generic Constraints: The activation grid](/reference/generic-constraints/#the-activation-grid)
...
```json
{
  "$schema": "https://docs.cobre-rs.dev/schemas/generic_parameters.schema.json",
  "scalar_parameters": [
    {
      "id": 1,
      "name": "rho_eq_h1",
...
The top-level JSON key stays `"scalar_parameters"` — only the file's location
and name changed.
...
The productivity and stored-energy quantities these tags resolve to are defined in
...
- For `per_stage_block`: `block_values` must be present, and each
  `(stage_id, block_id)` pair within an entry must be unique; duplicates are
  rejected.
- Unknown JSON fields on any entry are rejected immediately.

See [Generic Constraints: the five parameter kinds](/reference/generic-constraints/#generic_parametersjson-the-five-parameter-kinds)
