...
| `training.stopping_rules`  | array or null   | `[{ iteration_limit: 100 }]`        | Optional list of stopping-rule entries; when omitted or empty, the binary applies a single `iteration_limit` rule of 100 iterations |
...
:::note[`stopping_rules` is optional]
Earlier documentation described `training.stopping_rules` as required and said it
must include an `iteration_limit` rule. The current binary treats the array as
optional: when `stopping_rules` is omitted or empty, it applies a single
`iteration_limit` rule of 100 iterations. This page follows the binary — supply
your own rules to override that fallback.
:::
