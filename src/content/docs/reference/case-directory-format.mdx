...
| `training.parallelism`     | object or null  | `{backward_scheduler: trial_point}` | Backward-pass scheduler selection; see [Configuration](/running/configuration/#parallelism)                                |
...
| `past_anticipated_commitments` | No | Array of windowed committed-MW records for anticipated thermals, pre-study (see below) |
| `future_anticipated_deliveries` | No | Array of windowed delivery-bound records for anticipated thermals, post-horizon (see below) |
...
**`past_anticipated_commitments`** supplies the pre-study committed MW rate for
an anticipated thermal plant — the left-boundary counterpart to
`future_anticipated_deliveries` below. Each entry is a dated window carrying a
constant rate:
...
inferred. Each `value_mw` must lie within the plant's `[min_generation_mw,
max_generation_mw]` bounds and, if the plant declares a commissioning window,
mature inside it. The values are **sunk cost**: they never enter the study
objective. Windows for the same `thermal_id` must not overlap (adjacent
ranges where `start_date == previous end_date` are accepted). Optional;
...
**`future_anticipated_deliveries`** declares in-study decided commitments for
an anticipated thermal plant, delivered **after the study horizon** — the
right-boundary counterpart to `past_anticipated_commitments` above:

| Field            | Type    | Description                                                                             |
| ---------------- | ------- | ----------------------------------------------------------------------------------------- |
| `thermal_id`     | integer | Anticipated thermal plant identifier; must reference a thermal declaring `anticipated_config` |
| `delivery_start` | string  | Start of the delivery window (inclusive), ISO 8601 YYYY-MM-DD                             |
| `delivery_end`   | string  | End of the delivery window (exclusive), ISO 8601 YYYY-MM-DD; must be after `delivery_start` |
| `min_mw`         | number  | Lower bound of the delivered MW rate over the window; finite                              |
| `max_mw`         | number  | Upper bound of the delivered MW rate over the window; finite, `>= min_mw` (`min_mw == max_mw` pins a fixed commitment) |

Windows for the same `thermal_id` must not overlap (adjacent ranges accepted).
Each window must resolve into **exactly one** declared post-study stage in
`post_study_stages.json` — see [`post_study_stages.json`](#post_study_stagesjson)
below and [Post-Study Boundary](/math/post-study-boundary) for the boundary
formulation. Optional; defaults to an empty array when absent. A non-empty
`future_anticipated_deliveries` with no `post_study_stages.json` file present
is rejected at load.
...
```json
{
  "future_anticipated_deliveries": [
    {
      "thermal_id": 2,
      "delivery_start": "2026-09-05",
      "delivery_end": "2026-09-12",
      "min_mw": 0.0,
      "max_mw": 350.0
    }
  ]
...
per-`(thermal, post-study stage)` cost and delivery-bound table that
`future_anticipated_deliveries` windows (declared in
[`initial_conditions.json`](#initial_conditionsjson) above) resolve against by
date. Optional and inert when absent; **required** once any
`future_anticipated_deliveries` window is declared — a non-empty array with no
`post_study_stages.json` file present is rejected at load. See
...
and never carry Benders cuts — they exist solely to give
`future_anticipated_deliveries` windows a calendar to resolve against. No two
stages may share a `start_date`.
...
No two rows may share a `(thermal_id, post_study_stage_index)` pair. Each
[`future_anticipated_deliveries`](#initial_conditionsjson) window must resolve
into exactly one declared post-study stage with a matching `thermal_bounds`
row whose delivery capability intersects the commitment interval.
