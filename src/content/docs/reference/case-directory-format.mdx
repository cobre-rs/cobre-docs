...
| `block_id`             | INT32 (nullable) | No | Zero-based block index within the stage, selecting one block's turbined/outflow/generation/diversion override. `null` applies at the stage level. A non-null `min_storage_hm3`/`max_storage_hm3`/`filling_min_rate_m3s`/`water_withdrawal_m3s` value on the same row is rejected at validation — those four stay stage-level only, with no per-block variant. |
