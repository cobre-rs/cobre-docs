...
| `reference_date` | int32  | The `HydroInflowLag` reference past-stage `start_date`, encoded `YYYYMMDD` (`year*10000 + month*100 + day`); sentinel `-2147483648` (the int32 minimum) for every other family. |
| `interval_start` | int32  | Forward-family half-open window inclusive start, encoded `YYYYMMDD`: a `HydroTransitBucket` arrival start or an `AnticipatedThermalState` delivery start; sentinel `-2147483648` for `HydroStorage` and `HydroInflowLag`. |
| `interval_end`   | int32  | The half-open window exclusive end, paired with `interval_start`; sentinel `-2147483648` for `HydroStorage` and `HydroInflowLag`. |
...
| ----------------------- | ------- | -------- | ------------------------------------------------------------------------------------------------------------------- |
| `scenario_id`           | Int32   | No       | Scenario id (0-based). Duplicates the Hive partition directory (`scenario_id=NNNN/`) as an explicit in-file column. |
| `stage_id`              | Int32   | No       | Declared stage id (from `stages.json`) at which the lane's commitment was deposited.                                                 |
| `node_id`               | Int32   | No       | Visited policy-graph node id; equals `stage_id` on a stage chain.                                                   |
