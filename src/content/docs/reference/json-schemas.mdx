...
`https://methodology.cobre-rs.dev/schemas/config.schema.json` once the site
  ...
  "json.schemas": [
    {
      ...
      "url": "https://methodology.cobre-rs.dev/schemas/config.schema.json"
    },
    {
      "fileMatch": ["system/hydros.json"],
      "url": "https://methodology.cobre-rs.dev/schemas/hydros.schema.json"
    }
  ]
}
```
...
```json
{
  "$schema": "https://methodology.cobre-rs.dev/schemas/config.schema.json",
  "training": {
    "forward_passes": 192,
...
Until `methodology.cobre-rs.dev` is serving this site, the identical schema
content is also available straight from the tagged `cobre` repository, e.g.
`https://raw.githubusercontent.com/cobre-rs/cobre/v0.9.0/book/src/schemas/config.schema.json`.
:::
...
copy (currently at the `v0.9.0` tag) under `public/schemas/` so the files
