...
The schema lives at
[`crates/cobre-io/schemas/policy.fbs`](https://github.com/cobre-rs/cobre/blob/v0.16.0/crates/cobre-io/schemas/policy.fbs)
...
[`policy.fbs`](https://github.com/cobre-rs/cobre/blob/v0.16.0/crates/cobre-io/schemas/policy.fbs)
...
breaking change, reclaimed one field id and permanently burned three others:
...
- **`EntitySlot.delivery_anchor` (id 4)** is marked `deprecated`. It held a
  former month-integer anchor (`year * 12 + (month - 1)`).
- **`EntitySlot.delivery_date` (id 5)** is **also** marked `deprecated`. It held
  a former single `YYYYMMDD` calendar date, replaced by a per-family triple:
  **`reference_date` (id 6)** — the reference past-stage `start_date`
  (`YYYYMMDD`) of a `HydroInflowLag` slot; **`interval_start` (id 7)** /
  **`interval_end` (id 8)** — the half-open forward-family window (a
  `HydroTransitBucket` arrival window or an `AnticipatedThermalState` delivery
  window). Each date field reads sentinel `-2147483648` (the int32 minimum) where a
  slot's family carries no such date. The burn rule is **per-table**:
  `EntitySlot`'s id 7 (`interval_start`) is a **live** field, distinct from the
  burned `AffinePiece.reserved_7` (also id 7).
...
(`FORMAT_VERSION` is `2`). Either way there is **no conversion path**: the only
...
- **Removing a field** → mark it `deprecated`, never reuse the id. See
  `AffinePiece.reserved_7`, `EntitySlot.delivery_anchor`, and
  `EntitySlot.delivery_date` for worked examples. `AffinePiece.intercept`
  reclaiming id 4 from the former
