...
**stage id** for `basis/` and `states/`. The reader derives identity from
inside each buffer, never from the file name.
...
below. `policy/metadata.json` carries a required `format_version` marker,
checked **before any `.bin` payload is parsed**, and every `.bin` payload
carries the `CBVF` `file_identifier` described above. A checkpoint written
by an earlier release has neither, and fails to load with a named error
reporting the expected `format_version` against the value found —
surfaced as `cobre.errors.OutputError` in Python, and as a CLI message
prefixed `failed to read policy checkpoint: …`. There is **no conversion
path**: the only remedy for a checkpoint written by an earlier release is
to retrain.
