...
| `1`  | Validation | `cobre run`: every non-I/O load failure (parse, schema, semantic constraint, policy compatibility). `cobre validate`: a `config.json` parse/schema error, or any semantic constraint violation |
...
| `4`  | Internal   | Communication failure, unexpected channel closure, or other software/environment problem. `cobre validate` also maps a parse, schema, or policy-incompatibility error found in any case file **other than `config.json`** (e.g., a malformed `system/*.json` or `scenarios/*`) here |
...
:::note
`cobre validate` and `cobre run` do not share one exit code for the same load
failure. `cobre validate` exits `1` for a `config.json` parse/schema error and
for a semantic constraint violation, `2` for an I/O error, and `4` for a parse,
schema, or policy-incompatibility error in any *other* case file; `cobre run`
exits `1` for every non-I/O load error. These pages follow the binary.
:::
