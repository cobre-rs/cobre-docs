...
| `provenance`    | `ProvenanceReport` or `None` | Build version and environment metadata (a `ProvenanceReport` typed dict, or `None` when provenance was not captured) |
...
result = cobre.run.run(
    ...
    threads=4,                      # default: None → 1 worker
    config_overrides={"simulation": {"enabled": False}},  # default: None
)
```

- **`output_dir`**: where to write results (default: `case_dir/output`).
- **`threads`**: worker count for the internal pool. Default `None` ⇒ **1 worker**. `0` is **rejected** (`ValueError: threads must be >= 1 when given`); a value `≥ 1` sets the worker count.
- **`config_overrides`**: a dict merged into `config.json`. Both nested `{"simulation": {"enabled": False}}` and flat dotted `{"training.tree_seed": 7}` shapes are deep-merged into the configuration.

## Validate a Case

Before running a study, validate the case directory to check for errors:

```python
import cobre.io

report = cobre.io.validate("path/to/case")
...
`Study(...).validate()` returns the same-shaped dict.

## Error Handling

Cobre defines a typed exception hierarchy under `cobre.errors`. All exceptions inherit from `CobreError` (the common base) and also from the matching builtin (`ValueError`, `OSError`, or `RuntimeError`), so existing `except OSError` handlers keep catching while new code can catch the typed class.

| Exception                  | Inherits                        | Description                                                                                         |
| -------------------------- | ------------------------------- | --------------------------------------------------------------------------------------------------- |
| `CobreError`               | `Exception`                     | Base class for every Cobre exception                                                                |
| `ValidationError`          | `CobreError`, `ValueError`      | Case data or configuration failed validation                                                        |
| `PolicyIncompatibleError`  | `CobreError`, `ValueError`      | A warm-start policy is incompatible with the current system                                         |
| `CaseIoError`              | `CobreError`, `OSError`         | A filesystem read or write failure while loading or writing a case                                  |
| `OutputError`              | `CobreError`, `OSError`         | An output serialization, schema, or manifest failure while writing results                          |
| `SolverError`              | `CobreError`, `RuntimeError`    | A training or solver failure. For an infeasible subproblem, the `.stage`, `.iteration`, and `.scenario` attributes are the integer coordinates; otherwise they are `None` |
| `SimulationError`          | `CobreError`, `RuntimeError`    | A simulation-phase failure                                                                          |
| `InternalError`            | `CobreError`, `RuntimeError`    | An internal software or environment fault                                                           |

See [Error Codes](/reference/error-codes/) for the full catalogue of error conditions.
...
| Key           | Type         | Description                                                             |
| ------------- | ------------ | ------------------------------------------------------------------------ |
| `metadata`    | `dict`       | `format_version`, `cobre_version`, `created_at`, `num_stages`, `graph_manifest`, `season_manifest`, `producer` |
...
carries follows from its family: a `HydroInflowLag` slot carries `reference_date`
(its reference past-stage `start_date`, encoded `YYYYMMDD`); the forward families
(`HydroTransitBucket`, `AnticipatedThermalState`) carry the half-open
`interval_start`/`interval_end` window (both encoded `YYYYMMDD`); every other slot reads the int32-minimum
sentinel (`-2147483648`) in all three fields.
...
`stage_cuts` and `metadata` mirror the `load_policy` shapes above, so a checkpoint
loaded from disk round-trips through Python without reshaping — with one caveat:
`active_cut_indices` is written by `write_policy_checkpoint` but not returned by
`load_policy`, so a load → write cycle resets it, and cut activity round-trips
through each cut's `is_active` flag instead. `inflow_lag_depth`, when set to
...
`write_policy_checkpoint` raises `ValueError` when any of the following holds.
...
- a cut's `coefficients` length does not match its stage's `state_dimension`;
- a stage's state data length does not match `count * state_dimension`;
- a cut carries `inflow_lag_coefficients` without a positive `inflow_lag_depth`;
- (under `inflow_lag_depth`) a manifest lacks a leading storage block; or
- (under `inflow_lag_depth`) an inflow-lag coefficient is unplaceable.
...
A cut carrying `inflow_lag_coefficients` with no `inflow_lag_depth` passed to
...
cobre.write_policy_checkpoint("output/policy", stage_cuts, metadata)
# ValueError: stage 0 cut 1: inflow_lag_coefficients supplied without inflow_lag_depth; pass inflow_lag_depth=N to reserve the lag slots
```

## Next Steps

- See [Case Directory Format](/reference/case-directory-format/) for input file specifications.
- Explore the [Worked Examples](/examples/toy-single-reservoir/) for small, fully worked cases.
