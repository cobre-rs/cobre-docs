...
:::tip[Running on an HPC cluster?]
The methods here install the single-process binary (`comm: local`). For
multi-node runs — MPI-enabled binaries, building MPICH from source, SLURM
launch, and AWS ParallelCluster — see
[HPC & Cluster Deployment](/running/hpc-deployment/).
:::
...
This within-version guarantee has a cross-release counterpart: the backward
pass's opening solve order can change between cobre releases (for example,
relative to v0.11.1, which used a fixed sigma-weighted order). On a stage with
**three or more openings**, a different opening order can settle on a different
equally optimal vertex of a degenerate problem, all of which are valid, so
training and simulation outputs on such a stage can shift across the release
boundary. Stages with fewer than three openings have no order to change and
...
On a CLP build, every `training.solver`/`simulation.solver` override field is
rejected at study setup — see
[solver profile validation](/reference/error-codes/#solver-profile-validation-sddperror)
...
- [Running Studies](/running/running-studies/) — validate, run, and inspect results for any case directory
