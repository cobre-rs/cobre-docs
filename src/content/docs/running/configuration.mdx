...
- Every stage uses the expectation risk measure (`stages.json`
  `risk_measure: "expectation"`; a `cvar` measure with `lambda: 0` is
  expectation-equivalent and also admissible).

Under `sampled` selection, or an effectively risk-averse measure at any
stage, the `gap` rule is rejected at load with a validation error — the
exact-bound comparison it depends on is unavailable in those cases.
