...
| ---------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `selection`      | object | Scenario-selection method for the training forward pass — `sampled{forward_passes}` or `enumerated{}` (see [`training.selection`](#trainingselection) below). No default: a missing `training.selection` is a hard load error. |
| `stopping_rules` | array  | Stopping rules (see below). The key is required; any non-empty set is accepted. Training never runs past the largest `iteration_limit` in the set, or 100 iterations when the set has none. An empty array `[]` is treated as a single `iteration_limit` rule of 100 iterations. |

### `training.selection`

Chooses how many trajectories the training forward pass runs per iteration,
or whether it exhaustively enumerates the scenario openings instead of
...
| ---------------- | ------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `method`         | string  | Yes            | `"sampled"` or `"enumerated"`                                                                                                                                          |
| `forward_passes` | integer | `sampled` only | Number of scenario trajectories per iteration. Larger values reduce variance in each iteration's cut but increase cost per iteration. Rejected under `"enumerated"`. |

There is **no default** forward-pass count: an absent `training.selection` is
a hard load error (`LoadError::SchemaError`, naming the field
`training.selection`, message "a forward-pass count is required via
training.selection"). The removed root-level `training.forward_passes` field
from pre-v0.14.0 releases is rejected as an unknown field
...
Example — enumerated (the forward pass exhaustively walks the scenario
openings instead of drawing a fixed count):
...
| `tree_seed`     | integer            | `null`  | Random seed for the opening scenario tree. When `null`, a default seed of 42 is used (deterministic but arbitrary). See [Seed resolution](/reference/case-directory-format/#seed-resolution) for how it relates to `scenario_source.seed`. |
...
| `cut_selection`   | object             | row selection disabled                | Row-management (cut-selection) pipeline, nested at `training.cut_selection` (not a root key). See [`cut_selection`](#cut_selection). |
| `solver`          | object             | retry policy, no per-phase overrides  | LP solver retry policy plus optional per-phase profile overrides. See [`training.solver`](#trainingsolver). |
| `parallelism`     | object             | `by_scenario` scheduling              | Backward-pass worker-scheduling knobs. See [`parallelism`](#parallelism). |
| `scenario_source` | object or `null`   | `null` (all `in_sample`)              | Per-class forward-pass noise source for the training forward pass. See [`scenario_source`](#scenario_source). |

For the per-class `scenario_source` configuration, see the
[`scenario_source` sub-section](#scenario_source) below and
[Scenario Generation §3](/math/scenario-generation#3-sampling-scheme-abstraction).
...
| Field              | Type              | Default       | Description                                                                                             |
| ------------------ | ----------------- | ------------- | ------------------------------------------------------------------------------------------------------- |
| `seed`             | integer or `null` | `null`        | Shared forward-pass seed; **required when any class is not `in_sample`** (`out_of_sample`, `historical`, or `external`). |
...
| `openings`         | object            | `{"source": "generated"}` | Backward opening-tree source. `{"source": "generated"}` builds the tree internally; `{"source": "file"}` reads a user-supplied tree from `scenarios/noise_openings.parquet` (sampled selection and chain-dialect `stages.json` only). Training only. |

Valid values for `"scheme"`: `"in_sample"`, `"out_of_sample"`, `"external"` for every class;
`"historical"` for `inflow` only.

Admission rules (checked at load; the exact messages are listed under
[Case Directory Format](/reference/case-directory-format/#configjson)):

- `historical` is valid for `inflow` only — `load` or `ncs` on `historical` is rejected.
- `seed` is required when any class is not `in_sample`.
- `historical_years` is rejected unless some class uses `historical`.
- A `historical_years` range must have `from <= to`.
- `openings` is accepted under `training.scenario_source` only; under
  `simulation.scenario_source` it is rejected.

Under the `"external"` scheme a class's mean and standard deviation are derived
from its external scenario file itself, so that class's seasonal-statistics
input (`scenarios/inflow_seasonal_stats.parquet`, `load_seasonal_stats.parquet`,
or `non_controllable_stats.parquet`) is **optional** — it is not consulted when
the class is external everywhere. A constant (σ = 0) external column is accepted
for load, NCS, and an order-0 inflow model; for an autoregressive inflow model
of order `> 0` it is rejected (see [Error Codes](/reference/error-codes)).
...
See [Scenario Generation §3](/math/scenario-generation#3-sampling-scheme-abstraction)
for a full description of each scheme and of historical windows.
...
Stop when the exact upper bound has closed to within tolerance of the lower
bound. Requires an enumerated training forward pass, so the upper bound being
compared is exact rather than a statistical estimate.
...
| Field                | Type  | Description                                                                                                                                        |
| -------------------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tolerance`          | float | Optional. Absolute gap tolerance, in canonical R$.                                                                                                  |
| `relative_tolerance` | float | Optional. Relative gap tolerance in **percent** — `0.01` means 0.01%, compared against `100 · gap / max(1, \|lower_bound\|)`.                       |

At least one of `tolerance` / `relative_tolerance` must be present; a `gap`
rule with neither field is a load-time error: "gap stopping rule requires at
least one of tolerance / relative_tolerance to be present". When both are
present, the two arms combine by **disjunction** — training stops as soon as
...
- [`training.selection`](#trainingselection)`.method` is `"enumerated"` (the
  forward pass exhaustively enumerates the scenario tree, so the upper bound
  is exact rather than a sampled estimate).
- The risk measure is **uniform across every stage**: either
  `risk_measure: "expectation"` at every stage, or the same `cvar` measure
  (identical `lambda` and `alpha`) at every stage in `stages.json` (a `cvar`
  with `lambda: 0` is expectation-equivalent). Under a uniform `cvar` the
  exact upper bound is computed as a nested, time-consistent risk recursion
  over the enumerated tree, so it still brackets the risk-averse lower bound.

Under `sampled` selection, or a **stage-varying** measure (the risk measure
differs across stages), the `gap` rule is rejected at load with a validation
error — the exact-bound comparison it depends on is unavailable in those
cases.

:::caution[Migrating from the removed `simulation` stopping rule]
The `simulation` stopping rule has been **removed**: a stopping rule with
that `type` is now rejected at load as an unknown variant, not silently
ignored. Replace it with `gap` (under enumerated selection) or
`bound_stalling`.

`relative_tolerance` is a **percent**, not a fraction: `0.01` means 0.01%.
Carrying over an old fractional value unchanged is a 100× tighter reading
than intended and will produce a far tighter — likely unreachable —
stopping threshold.
:::

:::note[Type-scoped fields]
As with every JSON input, unknown keys are rejected — for stopping rules specifically, each rule object accepts only the fields belonging to its own `type`. Supplying a parameter that belongs to a different rule type (for example, `seconds` on an `iteration_limit` rule) is now a load-time error, not a silently ignored key. The same type-scoped strictness applies to `training.selection`, `simulation.selection`, and `training.parallelism.backward_scheduler`.
:::
...
Full path: `training.cut_selection`.
...
| `selection` | object  | `null`  | Scenario-selection method for the simulation phase — `sampled{num_scenarios}` or `enumerated{}` (see [`simulation.selection`](#simulationselection) below). Absent resolves to a sampled count of `2000`. |

### `simulation.selection`

Chooses how many trajectories the post-training simulation runs, or whether
it exhaustively enumerates the scenario set instead of sampling. Internally
tagged on `method`, mirroring [`training.selection`](#trainingselection)
above. Unlike `training.selection`, this field is **optional**: an absent
`simulation.selection` resolves to a sampled count of `2000` — the same
default the removed flat `num_scenarios` field carried before this field
existed.

| Field           | Type    | Required          | Description                                                                                                       |
| --------------- | ------- | ------------------ | --------------------------------------------------------------------------------------------------------------- |
| `method`        | string  | Yes (if present)  | `"sampled"` or `"enumerated"`                                                                                     |
...
The scheme values and admission rules are those of [`training.scenario_source`](#scenario_source); `openings` is rejected here.
...
:::note[Accepted but not yet consumed]
The `checkpointing` block (`enabled`, `initial_iteration`, `interval_iterations`,
`store_basis`, `compress`) is accepted and validated, but the current Cobre binary
has no consumer for it — **periodic** checkpointing is not performed. A single
policy checkpoint is always written at the **end** of training regardless of these
keys (see [Policy Management — Resume](/running/policy-management/#resume)). These
pages follow the binary.
:::

| Field                 | Type    | Default | Description                                                         |
| --------------------- | ------- | ------- | ------------------------------------------------------------------- |
| `enabled`             | boolean | `false` | Accepted; currently inert — no periodic checkpointing is performed. |
...
| `store_basis`         | boolean | `false` | Accepted; currently inert — basis output is not gated by this key.  |
...
stage-index knob. `strict` governs a source that prices _more_ than this study
models.
...
Example — load a monthly policy as the terminal boundary:
...
Running `cobre validate --json` on a case that configures a boundary emits the
...
Here one anticipated-commitment slot the source priced (plant `17`, delivering
...
for a full explanation of the coupling workflow, and
[Compatibility requirements](/running/policy-management/#compatibility-requirements)
for the date-selection and subset rejects.
...
| Section                          | Purpose                                                                                                  |
| --------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `upper_bound_evaluation`         | Inner approximation upper-bound evaluation settings. Accepted and validated, but currently inert — the engine evaluates the upper bound with its built-in defaults and does not consult this block. |
...
:::note[Retry knobs accepted but not yet consumed]
`retry_max_attempts` and `retry_time_budget_seconds` are accepted and validated
but currently inert — the solve path does not retry on these settings. The rows
below and the example JSON remain valid input and round-trip unchanged. The
per-phase `backward` / `forward` profile overrides are consumed as documented.
:::
...
`simulation.solver` (see [`simulation`](#simulation) above) is a third,
