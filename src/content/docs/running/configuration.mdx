...
- `historical_years` is rejected unless some class uses `historical`.
- A `historical_years` range must have `from <= to`.
- `openings` is rejected here — it is accepted under `training.scenario_source` only.
...
Example — load a monthly policy as the terminal boundary (the source pool priced
at the study's terminal date is selected automatically):
