...
to reduce the magnitude of objective coefficients, improving simplex
numerical stability.
...
double-scaling the already-conditioned matrix. This is overridable per
phase via the solver-profile [`scale`](/running/configuration/) field:
selecting `solver_scaling` re-enables solver-side (Curtis–Reid) scaling
and forgoes the exact single-unscaling guarantee (see
[LP Scaling §12.3 (Row Scaling)](/math/lp-formulation#123-row-scaling-geometric-mean)).
...
#### Opening-Block Scheduling

By default, the backward pass claims one whole trial point per work unit (see
above). The `opening_block` scheduler shrinks the work unit to one
...
for the config field that enables `opening_block` and sets its `block_size`.

:::caution[Opening-block scheduling and Dynamic Cut Selection]
The `opening_block` path depends on the frozen-LP backward setup, which is
...
**always** falls back to per-trial-point scheduling — so from DCS's
`start_iteration` (default `2`) onward, configuring `opening_block` has
...
not make `opening_block` a no-op.
:::

### Per-Phase Solver Profiles

Each algorithmic phase — forward sweep, backward sweep, and simulation — is
configurable independently via the `training.solver.backward`,
`training.solver.forward`, and `simulation.solver` overrides (see
[`training.solver`](/running/configuration/#trainingsolver)), which set the LP
solver's feasibility tolerances, per-attempt iteration caps, pricing
strategy, and more.

All three phases ship with an identical built-in profile, and that profile
differs from the LP backend's own default only in the simplex pricing
strategy — `row_hyper_sparse` instead of the backend's default `row` — to
exploit the sparse cut-subgradient rows. An absent override therefore leaves
a phase at this built-in profile, not the backend's default.

Overriding the backward phase's profile via `training.solver.backward` with
tighter feasibility tolerances or a stricter iteration cap can still reduce
backward-pass solve-time variance, which in turn improves load balance
across worker threads and shortens wall-clock training time.

The `scale` override carries a single-unscaling-exactness trade-off; see
[Cost Scale Factor](#cost-scale-factor) above.
