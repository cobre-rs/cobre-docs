...
| Transmission capacity       | Per-stage/per-block line bounds (`direct_mw`/`reverse_mw`) | `constraints/line_bounds.parquet` |
| VminOP / electric / AGRINT  | Generic LP constraints   | `constraints/generic_constraints.json`  |
| VminOP storage coefficients | Named constraint parameters (`@rho_acum`) | `constraints/generic_parameters.json` |
...
output/
  ...
  constraints/
    ...
    generic_parameters.json               (named @parameters, e.g. VminOP ρ_acum)
