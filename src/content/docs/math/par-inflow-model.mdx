...
The data model stores **seasonal sample statistics** and **standardized AR coefficients**. The residual std ratio is not a third stored quantity — it is derived from the AR coefficients alone (see "Deriving the residual std ratio" below). The relationship between stored and derived quantities is:

Stored vs. derived quantities — the files on disk hold the scale-invariant
`ψ*` alone; at runtime the residual std ratio is derived from `ψ*` by a
periodic-ACF closure, then everything is converted to original-unit `ψ` and
`σ` using the seasonal stats, and consumed by the LP stage subproblem.
...
- **Dynamics plane** — the standardized coefficients $\psi^*_{m,\ell}$ and the
  residual std ratio $r_m = \sigma_m/s_m$ (both dimensionless): the _shape_ of
  the temporal dependence, independent of magnitude. Only $\psi^*_{m,\ell}$ is
  an independent parameter here — $r_m$ is pinned by $\psi^*_{m,\ell}$ itself.
...
coefficients and the noise while preserving the correlation structure. Note the
**standardization basis**: $\psi^*$ is the coefficient of the process normalised
by the **seasonal std $s_m$**, not by the innovation std $\sigma_m$ — the two
normalisations differ whenever $r_m$ varies across seasons, and only the
$s_m$-normalised form re-couples against the conditioning plane. (This is the
distinction an externally-fitted model must respect — see the software layer's
Inputs & Outputs tab.)
...
runtime: "Runtime format — in-memory for LP" {
  ...
  sigma: "Original-unit residual std:  σₘ = sₘ · rₘ"
}
...
### Deriving the residual std ratio

Unlike $\mu_m$, $s_m$, and $\psi^*_{m,\ell}$, the residual std ratio $r_m$ is not read from a file. It is pinned by $\psi^*$ itself under the model's unit-marginal-variance contract — every season's standardized process $(a_{h,t}-\mu_m)/s_m$ has unit variance — via a **periodic-ACF closure**.
...
No further autocorrelation values are needed beyond the closure derivation above. All required quantities — $\psi_{m,\ell}$, $\sigma_m$, and the $r_m$ they depend on — are derived solely from the stored seasonal stats and AR coefficient file, with no historical data and no separate residual-ratio input.
...
The Yule-Walker solution $\psi_{m,\ell}^*$ is in standardized form — the direct output of step 3. It is stored as-is in `inflow_ar_coefficients.parquet`. No conversion to original units is performed, and no other quantity is written alongside it: the residual std ratio is not a fitting output. It is derived from $\psi^*$ afterward, uniformly for every model regardless of its origin, by the periodic-ACF closure of section 3 — not computed here.

For a season whose AR order is shared by every season of the cycle, that closure value coincides exactly with this step's own Yule-Walker system:
...
The residual standard deviation for season $m$ is recovered at runtime from the closure-derived residual std ratio $r_m$ (see section 3):

$$
\hat{\sigma}_m = \hat{s}_m \cdot r_m
$$

For a season whose AR order is shared by every season of the cycle, section 5.5 showed this reduces to the fitting system's own closed-form expression:
...
4. **Low residual variance** (warning): $r_m^2 < 0.01$ (the AR fit explains more than 99% of the season's variance) raises an advisory model-quality warning; it is not fatal.
...
- $r_{m(t)} = \texttt{residual\_std\_ratio} = \sigma_{m(t)}/s_{m(t)} \in (0, 1]$: the standard deviation of the **standardized** innovation — the closure-derived value of section 3, $\sqrt{1 - \sum_\ell \psi^*_{m,\ell}\,\rho_{m(t)}(\ell)}$, with $\rho_{m(t)}$ the model's implied periodic ACF
- $\varepsilon_t \sim \mathcal{N}(0, 1)$: unit-variance innovation noise

The innovation of the $s_m$-standardized process is **not** unit-variance: because that process has unit _marginal_ variance, its one-step innovation has standard deviation $r_{m(t)} \le 1$ — which is exactly why the residual std ratio appears here explicitly. Normalising by the seasonal std $s_m$ (as stored) is deliberately **not** the same as normalising by the innovation std $\sigma_m$: the two bases give coefficients differing by a factor $r_{m-\ell}/r_m$, and only the $s_m$-standardized coefficients are the stored $\psi^*$ (see the two-plane note in section 3). The next step converts $\psi^*_{m,\ell}$ to original-unit $\psi_{m,\ell}$ for use in the LP.
...
The residual standard deviation is also derived at this preprocessing step, from the closure-derived residual std ratio $r_m$ (section 3):
