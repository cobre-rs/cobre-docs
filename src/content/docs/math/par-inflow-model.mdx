...
- **Dynamics plane** — the standardized coefficients $\psi^*_{m,\ell}$ and the
  ...
  the temporal dependence, independent of magnitude.
...
2. **Residual std ratio range**: each `residual_std_ratio` must be finite and lie in $(0, 1]$.
3. **Residual std ratio consistency**: the value must be identical across all lag rows sharing the same (hydro_id, stage_id) group.

**Enforced when validating the assembled model:**

4. **Positive sample std** (error): a season with AR order $> 0$ must have $s_m > 0$ — a zero seasonal std cannot normalise the AR coefficients.
5. **Low residual variance** (warning): $\texttt{residual\_std\_ratio}^2 < 0.01$ (the AR fit explains more than 99% of the season's variance) raises an advisory model-quality warning; it is not fatal.

**Enforced during fitting, not as a post-hoc test:**

6. **Stability**: rather than testing the roots of the AR polynomial, the fitter guards stability through the Maceira & Damázio contribution-based order reduction (section 4.1) — a negative lag-1 coefficient, a negative composed contribution, or a coefficient exceeding `max_coefficient_magnitude` reduces the season's order and refits. The periodic Yule-Walker solve additionally truncates the order when its correlation matrix is singular (section 5.4), so a rank-deficient system never yields coefficients.
...
- $r_{m(t)} = \texttt{residual\_std\_ratio} = \sigma_{m(t)}/s_{m(t)} \in (0, 1]$: the standard deviation of the **standardized** innovation — equivalently $\sqrt{1 - \sum_\ell \psi^*_{m,\ell}\,\hat\rho_m(\ell)}$ (section 5.5)
