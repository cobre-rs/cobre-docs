...
- [$\psi_{m(t),\ell}$](/overview/notation-conventions#35-inflow-model-parameters): Autoregressive coefficient for lag $\ell$ in season $m(t)$
- $\sigma_{m(t)}$: Innovation standard deviation for season $m(t)$ (**derived** at load — see section 3)
- [$\varepsilon_t$](/overview/notation-conventions#35-inflow-model-parameters) $\sim \mathcal{N}(0, 1)$: Innovation (standardized noise)
...
+ \underbrace{\sigma_m \cdot \varepsilon_t}_{\text{stochastic innovation}}
$$

where $a_{h,\ell}$ are state variables (lagged inflows) and $\varepsilon_t$ is the sampled noise realization.
...
The clipping rule is fixed and transparent — exactly the strictly negative eigenvalues are set to zero, nothing else is altered. The cross-entity correlation structure is preserved exactly for all non-negative eigenvalues.
