...
- $\eta_h$ = turbine efficiency (a dimensionless factor below 1), from the hydro object's `efficiency` field
- $H^{ref}_{h,t}$ = reference net head (meters), evaluated at the reference storage level and varying by stage
...
where $k_{loss}$ is a small dimensionless fraction of the gross head.
...
A hydro plant is partitioned into one or more **(hydro, bus) cells** — one per distinct bus among its declared unit groups (see [system elements §5](/math/system-elements)) — and FPHA is evaluated **per cell**, not per plant: a plant's fitted hyperplane set (§2.1–§2.8) is fitted once per plant and consumed by every one of its cells, apportioned as described below. A single-cell plant's constraint is byte-identical to the pre-partition, per-plant form.

#### Final FPHA Constraint

For each cell $(h, b)$ of a hydro $h$ using FPHA, block $k$, and plane $m \in \mathcal{M}_h$:

$$
g_{h,b,k} \leq \sigma_{h,b} \big( \gamma_0^m + \gamma_v^m \cdot v_h^{avg} + \gamma_s^m \cdot s_{h,k} \big) + \gamma_q^m \cdot q_{h,b,k}
$$
...
$\sigma_{h,b}$ apportions the plane's **flow-independent** part — the intercept $\gamma_0^m$, the storage term, and the spillage term — by cell $(h,b)$'s share of the plant's declared turbine capacity:

$$
\sigma_{h,b} = \frac{\sum_{g \,\in\, (h,b)} \bar{Q}_g}{\sum_{g \,\in\, h} \bar{Q}_g}
$$

($0$ when the plant's declared total is $0$, never `NaN`), a study-time constant derived from each unit group's declared `max_turbined_m3s` — never recomputed per stage or block, and never re-derived from a resolved per-stage override, which would double-count an availability derate already enforced on the flow path by each cell's own column bounds. It satisfies $\sum_{b \in \mathcal{B}_h} \sigma_{h,b} = 1$, and a single-cell plant has $\sigma_{h,b} = 1$ exactly. The flow coefficient $\gamma_q^m$ stays on the cell's own $q_{h,b,k}$ **unscaled** — it is the only term homogeneous in the cell partition, which is what makes $\sum_{b} g_{h,b,k}$ at fixed $v^{avg}_h$, $s_{h,k}$, and $\sum_b q_{h,b,k}$ recover the plant-level bound this per-cell form replaces.
...
where $v^{in}_h$ is the incoming storage LP variable (pinned to $\hat{v}_h$ via column bounds — see [LP Formulation §4a](/math/lp-formulation)) and $v_h$ is end-of-stage storage — a single plant-level quantity shared by every cell, since storage is not partitioned. Both are LP variables, so $v^{in}_h$ appears in every cell's FPHA constraint with coefficient $\sigma_{h,b} \cdot \gamma_v^m / 2$. The LP solver automatically accounts for this in the reduced cost of the pinned $v^{in}_h$ column.

#### Generation as Independent Variable

When using FPHA, the generation variable $g_{h,b,k}$ is **not** directly computed from turbined flow. Instead:

1. Generation is a free LP variable per cell, bounded by $[0, \bar{G}_{h,b}]$, where $\bar{G}_{h,b}$ sums the cell's own unit groups' declared `max_generation_mw` and closes against the plant's own resolved maximum (see [LP Formulation §6](/math/lp-formulation))
2. FPHA constraints (one per plane $m$, per cell) provide upper bounds relating generation to storage, flow, and spillage
3. The optimizer maximizes generation subject to FPHA constraints
4. At optimum, each cell's generation lies on one of its FPHA hyperplane facets
...
For hydros using **constant productivity**, the same regularization is applied uniformly so that the LP tie-breaks `(turbined, spillage)` decompositions deterministically. Applying it to constant-productivity plants — not to FPHA plants alone — keeps every turbine column priced, so the dispatch stays well-determined even on cases that mix the two production models.

For the full penalty taxonomy and priority ordering, see [Penalty System](/math/penalty-system).

### 2.11 Impact on Benders Cuts

The FPHA formulation affects water value computation. Because the incoming storage variable $v^{in}_h$ appears in **every one of the plant's cells'** FPHA constraints (via $v^{avg}_h = (v^{in}_h + v_h)/2$, each apportioned by that cell's $\sigma_{h,b}$, section 2.9), the FPHA hyperplane duals of every cell contribute to the marginal value of incoming storage. However, the implementation does **not** require manually combining duals from the water balance and FPHA constraints. Instead, pinning $v^{in}_h$ to $\hat{v}_h$ by column bounds (see [LP Formulation §4a](/math/lp-formulation)) makes its reduced cost capture the total sensitivity $\partial Q_t / \partial \hat{v}_h$ automatically — the LP solver propagates every cell's FPHA contribution through the single shared $v^{in}_h$.
...
This dual implicitly includes the water balance contribution ($\pi^{wb}_h$), the FPHA contribution summed over every cell of the plant ($\frac{1}{2} \sum_{b \in \mathcal{B}_h} \sigma_{h,b} \sum_m \pi_{h,b,m}^{fpha} \cdot \gamma_v^m$), and any generic constraint contributions — all resolved by the LP solver without explicit dual combination.
...
`linearized_head` is a **reserved production-model name**: it is equivalent to `constant_productivity` (§1) in every phase — training (backward and forward passes) and simulation alike — and selecting it applies no head correction and imposes no phase restriction.

Selecting `linearized_head` therefore yields dispatch identical to `constant_productivity`: generation is $g_{h,k} = \rho_{h,t} \cdot q_{h,k}$ with the plant's per-(hydro, stage) productivity (§5.1). It still requires the plant's productivity input.

## 4. Model Selection Guidelines

### Training (Policy Construction)

The substantive choice during training is between `constant_productivity` and `fpha` — `linearized_head` resolves to constant productivity (§3) and behaves identically to `constant_productivity`. That choice trades accuracy against computational cost:
...
During simulation, `constant_productivity` and `fpha` behave as they do in training, and `linearized_head` resolves to constant productivity (§3):
...
| Plants selecting `linearized_head`                       | Constant productivity (equivalent)  | `linearized_head` resolves to the same constant-productivity source (§3); no head correction is applied  |
...
**Physical storage range.** Each plant carries a physical storage range $[V^{min}_h, V^{max}_h]$: a stage-invariant property of the plant (its dead-volume floor and its full-reservoir ceiling). It is distinct from the per-stage operative bounds $[\underline V_h, \bar V_h]$ on the storage variable, which may be tighter at a given stage (for example under a flood-control ceiling). Every quantity in this subsection and in §5.4 reads the physical range, never the operative bounds, so an active operative restriction never truncates a stored-energy value.

**Mean forebay level.** The forebay level is averaged over the physical range:

$$
\bar h_{fore,h} \;=\; \frac{1}{V^{max}_h - V^{min}_h} \int_{V^{min}_h}^{V^{max}_h} h_{fore}(v)\, dv
$$

Because $h_{fore}$ is the piecewise-linear volume–height curve of §2.3, this integral is evaluated exactly (a composite trapezoid over the curve's breakpoints), not approximated by quadrature.

**Mean equivalent productivity.** With $h_{eq}(h_f, Q) = h_f - h_{tail}(Q) - h_{loss}$ the net-head function of §5.1, evaluated from a forebay level $h_f$ and the reference flow $Q$:

$$
\bar\rho_{eq,h,t} \;=\; \rho_{esp,h,t} \cdot h_{eq}\big(\bar h_{fore,h},\; Q^{ref}_{h,t}\big)
$$

It is the **forebay level** that is averaged, not the net head: the tailrace level and the hydraulic losses are then evaluated at $Q^{ref}_{h,t}$ exactly as the reference-point evaluator evaluates them. The two evaluators therefore differ only in the forebay level they feed to the same net-head function.

**Resolution order.** For each (hydro $h$, stage $t$):

1. An explicit equivalent-productivity override replaces **both** evaluators with the same value, so $\bar\rho_{eq,h,t} = \rho_{eq,h,t}$ equals the override.
2. Otherwise, a collapsed physical range ($V^{min}_h = V^{max}_h$), or a plant without volume–height geometry or without a specific productivity $\rho_{esp,h,t}$, gives $\bar\rho_{eq,h,t} = \rho_{eq,h,t}$: a zero-width range reduces the average to a point evaluation.
3. Otherwise, $\bar\rho_{eq,h,t}$ is the integrated value above.
...
**Mean accumulated productivity.** $\bar\rho_{acum,h,t}$ follows from the mean own terms by the same downstream recurrence as §5.2:

$$
\bar\rho_{acum,h,t} \;=\; \bar\rho_{eq,h,t} \;+\; \sum_{h' \,\in\, \text{downstream}(h)} \bar\rho_{eq,h',t}
$$

**Maximum stored energy.** The energy content of the plant's full useful volume is

$$
E^{max}_{h,t} \;=\; \bar\rho_{acum,h,t}\,\big(V^{max}_h - V^{min}_h\big)
$$

Its unit is the raw product MW/(m³/s) × hm³ — **not** MWh (no hm³-to-hours factor is applied).

**Security-curve identity.** A security curve is a per-stage floor on stored energy expressed as a fraction $p$ of the maximum stored energy. Written on the outgoing storage $v_h$ with both sides on the mean evaluator,

$$
\bar\rho_{acum,h,t}\,\big(v_h - V^{min}_h\big) \;\ge\; p\,E^{max}_{h,t}
\;\;\Longleftrightarrow\;\;
v_h - V^{min}_h \;\ge\; p\,\big(V^{max}_h - V^{min}_h\big)
$$

states "at least a fraction $p$ of the useful volume, in energy terms". The per-plant coefficient cancels only because both sides use the same (mean) evaluator and the same raw unit; mixing the reference-point coefficient on one side with $E^{max}_{h,t}$ on the other, or converting one side to MWh alone, would not reduce to the useful-volume fraction. How such a row is authored is described in [LP Formulation §10](/math/lp-formulation#10-generic-constraints).

### 5.4 Inflow and Storage in Energy Units

$\rho_{acum}$ and $\bar\rho_{acum}$ convert hydraulic quantities to energy units that downstream reporting expects:
...
$$
\text{EARM}^{\,\text{init}}_h \;=\; (V^{init}_h - V^{min}_h) \cdot \bar\rho_{acum,h,t} \cdot \frac{10^6}{3600}
\qquad
\text{EARM}^{\,\text{final}}_h \;=\; (V^{final}_h - V^{min}_h) \cdot \bar\rho_{acum,h,t} \cdot \frac{10^6}{3600}
$$

where $V^{min}_h$ is the **physical** minimum storage of §5.3 (not the stage's operative lower bound) and $\bar\rho_{acum,h,t}$ is the useful-range mean accumulated productivity. The conversion factor $10^6 / 3600$ converts hm³ to m³ and seconds to hours so that storage in hm³ multiplied by productivity in MW/(m³/s) yields MWh; no stage duration enters, so EARM is independent of stage length.

**Stored reservoir energy, power form** (MW):

$$
\text{EARM}^{\,\text{MW}}_h \;=\; \frac{\text{EARM}_h}{\sum_{k \in \mathcal{K}} \tau_k}
$$
...
| Hydro plant entity              | `specific_productivity_mw_per_m3s_per_m` (FPHA, and the useful-range mean evaluator for any model)      | $\rho_{esp}$ for $\rho_{eq}$ derivation (§5.1) and $\bar\rho_{eq}$ (§5.3)                                  |
...
| Hydro energy productivity input | Per-(hydro, stage) `equivalent_productivity_mw_per_m3s` and specific productivity                        | $\rho_{eq,h,t}$ override for both evaluators (§5.1 authoring source 2 + FPHA override path); $\rho_{esp,h,t}$ for both evaluators' head-derived value (§5.1, §5.3) |
| Hydro geometry                  | volume_hm3, height_m                                                                                     | $h_{fore}(v)$ interpolation (§2.3); mean forebay level $\bar h_{fore,h}$ (§5.3)                           |
...
<Tabs>
  ...
  <TabItem label="Inputs & Outputs">
    <HydroIO />
  </TabItem>
  <TabItem label="Implementation notes">
    <HydroNotes />
  </TabItem>
</Tabs>
