...
For hydros using **constant productivity**, the same regularization is applied uniformly so that the LP tie-breaks `(turbined, spillage)` decompositions consistently with NEWAVE. Applying it to constant-productivity plants — not to FPHA plants alone — keeps every turbine column priced, so the dispatch matches the reference model even on cases that mix the two production models. Plants using `linearized_head` (simulation-only, see section 3) are not subject to this regularization during training because training uses only `constant_productivity` and `fpha`; during simulation, the cost is irrelevant because no policy is being constructed.
