...
| `simulation/anticipated_lanes/` | Post-horizon commitment-lane results for anticipated thermals, keyed `(thermal_id, delivery_date)` — seven columns (`scenario_id`, `stage_id`, `node_id`, `thermal_id`, `delivery_date`, `deposited_decision_mw`, `carried_committed_mw`), Hive-partitioned by `scenario_id`. One row per declared window per terminal scenario; written only when a `future_anticipated_deliveries` window is declared. |
