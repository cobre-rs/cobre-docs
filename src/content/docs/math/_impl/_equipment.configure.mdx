...
Only `id`, `name`, `bus_id`, `cost_per_mwh`, and `generation` are required;
`entry_stage_id`, `exit_stage_id`, and `anticipated_config` are optional.
...
attaching the per-plant lead described in [system elements §4](/math/system-elements#4-thermal-plants):
...
| Field         | Type    | Description |
| ------------- | ------- | ----------- |
| `lead_stages` | integer | Number of stages of dispatch anticipation. A value of `2` means the commitment for stage `t` must be decided at stage `t - 2`. Must be `>= 1`, and (at load time) must not exceed the study's total stage count — a lead longer than the horizon could never deliver. |
...
`values_mw` must have exactly `lead_stages` entries, ordered oldest-to-most-recent.
...
```json
{
  "non_controllable_sources": [
    { "id": 0, "name": "Wind Farm A", "bus_id": 1, "max_generation_mw": 100.0 }
  ]
}
```
