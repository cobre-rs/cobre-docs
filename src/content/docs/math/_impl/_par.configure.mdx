...
| `min_observations_per_season`  | integer          | `30`     | Recommended minimum number of fully-covered stage occurrences of each season per hydro. Below this count, estimation still proceeds — see Implementation notes. |
...
| ----------- | -------- | ----------- |
| `method`    | Yes      | Correlation decomposition method. Accepts only `"spectral"` (eigendecomposition + clipped square root, methodology §8.2); `"cholesky"` is rejected as an unknown method. |
...
time. Correlation is modelled **independently per entity class** — inflow,
load, and NCS each carry their own spatial correlation structure, and there is
**no** cross-class correlation term.
