...
A delivered commitment's forward value is carried as ring-buffer state, so at
its delivery stage the must-generate coupling reads the solver's computed value
— accurate only to the LP backend's feasibility tolerance, not exact. Cobre runs
**no** per-solve reconciliation pass. Instead, the outgoing state is
**canonicalized onto its admissible bounds when it is read back** from each
solve, and that projection is the identity for a value already in box: a
commitment that settles a hair outside its cap is clamped back in-box before it
is carried forward, so training continues normally instead of surfacing a
spurious infeasible LP. This is the single read-back canonicalization seam the
methodology states once — see
[SDDP algorithm §3.1](/math/sddp-algorithm#31-forward-pass) and
[Determinism Guarantees §3](/math/determinism-guarantees#3-methodology-mechanisms).
...
infeasible LP surfacing mid-solve. See [Error Codes](/reference/error-codes) for
the failure class.
