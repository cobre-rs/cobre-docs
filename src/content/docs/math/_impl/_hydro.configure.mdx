...
Only `id`, `name`, `downstream_id`, `operational_start_date`, `reservoir`,
...
| `max_storage_hm3` | number | Maximum operational storage (flood control level). Must be greater than or equal to `min_storage_hm3` (equal is allowed — a fixed-storage plant). |
...
| Linearized head       | `"linearized_head"`       | Both phases                            | Reserved model name; the current engine resolves it to the same constant-productivity source as `constant_productivity` (see methodology §3). Requires the plant's productivity input. |
...
A plant's generation envelope is split into one or more turbine groups, each
...
group `id`s are unique within the plant. Across the whole plant, the sum of
every group's `max_turbined_m3s` (and, separately, `max_generation_mw`)
cannot exceed the plant's own declared maximum from the `generation` block
above — a plant's own value is the envelope, so groups cannot raise it — and
...
supplied by `constraints/hydro_unit_group_bounds.parquet` — see the Inputs &
Outputs tab.
...
When present, every field is optional and independently falls back to the
global default from `penalties.json`; the block accepts exactly these twelve
keys and rejects any unknown field. For the full field list, units, and the
priority ordering across penalty categories, see
[Penalty System](/math/penalty-system).
...
Beyond the entity-level diversion and spillage fields above,
...
- `min_diversion_m3s` — a minimum diversion-flow floor (m³/s). This is a
  **floor that requires a declared `diversion` channel** on the hydro (see
  Diversion channel, above): with no channel declared, the diversion column
  is pinned `[0, 0]`, so a positive floor is infeasible and is rejected at
  load.
- `min_spillage_m3s`, `max_spillage_m3s` — the spillage band (m³/s);
  `min_spillage_m3s` must be `<= max_spillage_m3s`.
...
[Case Directory Format → `constraints/hydro_bounds.parquet`](/reference/case-directory-format)
for the complete column table, including the pre-existing turbined/storage/
outflow/generation/diversion-maximum/filling/withdrawal override columns this
file also carries.
...
with `linearized_head` needs at least 2 rows; that higher minimum is a load-time
...
| -------------------------------- | ------- | ----------- |
| `volume_discretization_points`   | 5       | Number of volume grid points for fitting. Must be `>= 2`. Run-of-river plants (`γ_V` snapped to 0) bypass volume discretization — the fit collapses to a single volume point and emits one `hydro_geometry.parquet` row. |
...
:::note[Round-trip-only]
`spillage_discretization_points` is accepted and validated, but the current Cobre
FPHA fit is a convex hull of the (volume, turbined) cloud at zero spillage — it
has no spillage axis. The value is retained for input round-tripping and does not
change the fitted production planes. These pages follow the binary.
:::
