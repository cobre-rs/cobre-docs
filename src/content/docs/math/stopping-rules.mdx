...
- The forward pass [enumerates the scenario tree](/math/policy-graphs) exhaustively rather than sampling it, so the upper bound is exact — carrying no sampling error — rather than a sampled approximation.
- The risk measure is **uniform across all stages**: either expectation at every stage, or one CVaR measure (the same risk-aversion weight and tail level) at every stage. Under expectation the exact bound is the probability-weighted cost $\sum_{\ell} P(\ell)\, C(\ell)$ over every scenario $\ell$; under a uniform CVaR it is the nested, time-consistent risk bound over the same enumerated tree ([Upper Bound Evaluation §2](/math/upper-bound-evaluation)). A **stage-varying** measure — a risk-aversion weight or tail level that differs across stages — is not admissible, because no single measure then aggregates the tree.
