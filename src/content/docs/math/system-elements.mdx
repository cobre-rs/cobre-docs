...
| Aspect                 | Consequence                                                                                                    |
| ---------------------- | -------------------------------------------------------------------------------------------------------------- |
| **Objective function** | All cost terms are scaled by $\tau_k$: the coefficient for a thermal with marginal cost $c^{th}$ is $\tau_k \times c^{th}$ |
| **Variable bounds**    | Block-invariant — a capacity bound $\bar{g}$ is the same regardless of block duration                          |
...
Independent of the commissioning window, every entity also carries a
required `operational_start_date` — an ISO-8601 calendar date. This field is
**provenance and the canonical ordering key only**: it fixes the entity's
position in the canonical `(operational_start_date, id)` order (see
[Notation Conventions](/overview/notation-conventions) and
[Determinism Guarantees](/math/determinism-guarantees)), which every state
block, the LP column layout, and the output column order follow. It does
**not** compute or gate commissioning. Presence in the dispatch is decided
solely by the `entry_stage_id`/`exit_stage_id` window above: the two fields
are independent, and neither is derived from the other.
...
:::note[Example]
Real-world dispatch studies often model buses at the scale of major interconnection regions, but finer decompositions are equally valid.
:::
...
- **Source bus**: Exports $f^+_{l,k}$, receives $f^-_{l,k}$
- **Target bus**: Receives $f^+_{l,k}$, exports $f^-_{l,k}$

### Key Parameters

| Parameter     | Units  | Description                                                                |
| ------------- | ------ | -------------------------------------------------------------------------- |
| $\bar{F}^+_l$ | MW     | Capacity limit (direct direction); may vary by stage and, optionally, by block |
| $\bar{F}^-_l$ | MW     | Capacity limit (reverse direction); may vary by stage and, optionally, by block |
...
The exchange cost is a **regularization term** — a small per-unit cost on interchange flow — that prevents degenerate solutions with unnecessary power circulation and guides the solver toward physically meaningful flow patterns.
...
**Load balance contribution** at source bus: $-f^+_{l,k} + f^-_{l,k}$
...
Thermal costs represent actual operating expenses, which vary by fuel type, and constitute the primary controllable cost in the objective function.
...
#### Right-boundary carry

Absent a terminal boundary, the horizon predicate above is unconditional: a commitment whose delivery stage would fall outside the study horizon is pinned to zero, because there is nothing past $T$ to price it against. When a terminal boundary future-cost function is loaded, a commitment decided in-study but delivered after the horizon is no longer suppressed this way. It is held live instead: the same ring carries it, over a **delivery axis extended across the post-study calendar** — not a separate carrier — so the post-horizon commitment survives into the terminal stage's incoming-state vector and is priced through the boundary's affine floor on the terminal future-cost variable rather than pinned away. The commitment's delivery-anchored fuel cost is booked separately, on its own decision column at the stage it is decided (the same column the in-study objective already prices), so the carried-state pricing and the fuel booking compose without double-counting the same delivered energy. How much the plant may commit to a post-horizon delivery is bounded and priced by the **post-study delivery capability the boundary segment declares for that stage, alone** — there is no second, decision-side interval to intersect it with; a commitment reaching past what the boundary calendar can place has no stage to anchor to. See [Post-Study Boundary](/math/post-study-boundary) for the boundary formulation and [LP formulation §5c](/math/lp-formulation#5c-anticipated-thermal-dispatch) for the column mechanics that carry and price this lane.

#### Pre-horizon seed

Because anticipated dispatch carries state across stages, plant $i$ must be primed with $K_i$ pre-horizon commitments — the values that were committed before the study begins and are scheduled to mature at stages $0, \ldots, K_i - 1$. The seed is a set of **dated, delivery-anchored commitment windows**: each window declares an externally-decided MW rate held constant over the calendar span it covers, and the collection of windows must **tile the plant's leading delivery stages exactly** — every stage from $0$ to $K_i - 1$ falls inside exactly one window, with a committed rate of zero written explicitly for any stretch with no scheduled commitment rather than left to be inferred from a gap. Both the generation-bounds check and the coverage check are enforced by the semantic validator at load. A plant with no pre-horizon commitments to declare needs no windows at all — the default for studies with no anticipated plants. The same windows also carry the plant's decisions that were made before the study but **deliver after the horizon** (the _já-comandada_ case): those are not an in-study decision at all but a sunk cost, priced against the terminal boundary — see [Post-Study Boundary](/math/post-study-boundary). The concrete window shape is a software-layer concern; see [Case Directory Format](/reference/case-directory-format) and [equipment formulations](/math/equipment-formulations) rather than this conceptual layer.

The seed enters the LP via the slot-0 fishing constraint at stages $0, \ldots, K_i - 1$: at each such stage $t$, slot 0 is fed by the rate declared by whichever window covers delivery stage $t$ — resolved **by date**, not by position in a list — and the ring-buffer shift carries the next leading stage's covering rate into slot 0 in turn. From stage $K_i$ onward, slot 0 carries the LP-decided commitment $d^i_{t - K_i}$.
...
A plant whose groups all share one bus has exactly one cell and reduces to the single-bus model used throughout the rest of this chapter — the case for every plant in the system today except one. A plant whose groups span several buses has one turbined-flow variable and one generation variable per cell instead of per plant; see [LP formulation §3–§4](/math/lp-formulation) for how a cell's generation enters the bus load balance and a plant's cells sum into its water balance, and [LP formulation §6, §8](/math/lp-formulation) for how a cell's bounds compose from its member groups.
...
| $q_{h,b,k}$  | m³/s  | Turbined flow at cell $(h,b)$ (§ Unit Groups above), block $k$; the plant total $q_{h,k} = \sum_b q_{h,b,k}$ is what enters the water balance |
...
| $g_{h,b,k}$  | MW    | Hydro generation at cell $(h,b)$, block $k$; injects at bus $b$ (§ Unit Groups above) |
...
- **Bus connection**: Each of a plant's cells connects to its own bus for energy delivery; a single-cell plant connects to exactly one bus, as before (§ Unit Groups above)
- **Cascade topology**: Upstream plants' outflows ($q + s + u$, with $q$ the plant's total turbined flow summed over its cells) become downstream plants' inflows, either instantaneously or, on a declared main-cascade arc, delayed by a **water travel time** (see _Cascade Travel Time_ below)
...
| $\bar{Q}_{h,b}$, $\underline{Q}_{h,b}$           | m³/s      | Turbined flow bounds (machine limits), composed per cell from its member unit groups' declared bounds — see [LP formulation §8](/math/lp-formulation) |
| $\bar{O}_h$, $\underline{O}_h$                   | m³/s      | Outflow bounds (environmental flow, flood control)      |
| $\bar{G}_{h,b}$, $\underline{G}_{h,b}$           | MW        | Generation bounds (user-defined per unit group, not derived from flow), composed per cell the same way — see [LP formulation §6](/math/lp-formulation) |
...
**Horizon limitation.** In-transit water that would mature after the study's last stage is dropped rather than credited to terminal storage — but only **when no terminal boundary future-cost function is loaded**. When a [terminal boundary](/math/post-study-boundary) is loaded instead, this drop is lifted: the terminal deep-lag in-transit buckets are held live rather than capped away, carried into the terminal stage's incoming-state vector, and priced at the boundary through the same reduced-cost cut mechanism the buckets already use — the still-in-transit water is valued rather than discarded, with no residual "deeper-than-horizon" drop once the boundary is in play. See [LP formulation §5d](/math/lp-formulation) for the bucket-level mechanics and [Post-Study Boundary](/math/post-study-boundary) for the boundary formulation.

### Production Function (Water to Power)

Cobre supports three production-model names for converting turbined flow to electrical generation:

1. **Constant Productivity**: $g_{h,b,k} = \rho_h \cdot q_{h,b,k}$ (per cell $(h,b)$) — simple linear relationship with fixed $\rho_h$ [MW/(m³/s)], suitable for plants with stable head.

2. **Linearized Head** (reserved alias): A reserved model name that resolves to **constant productivity** in every phase — training and simulation alike. No distinct head-dependent model is applied; requesting it yields the constant-productivity model above. See [hydro production models §3](/math/hydro-production-models).

3. **FPHA (Função de Produção Hidrelétrica Aproximada)**: Piecewise-linear approximation via hyperplanes that captures head variation with storage level and accounts for tailrace effects from spillage. Fitted once per plant and evaluated per cell $(h,b)$, apportioning the plane's flow-independent terms by the cell's share of the plant's turbine capacity (see [hydro production models §2.9](/math/hydro-production-models)). Each plane $m$:

$$
g_{h,b,k} \leq \sigma_{h,b}\big(\gamma_0^m + \gamma_v^m \cdot v^{avg}_h + \gamma_s^m \cdot s_{h,k}\big) + \gamma_q^m \cdot q_{h,b,k}
$$

:::note[Training models]
All three model names are accepted for both training (policy construction) and simulation (policy evaluation). A `linearized_head` request resolves to the constant-productivity model in every phase — no production model is restricted to a single phase.
:::
...
| $q_{h,b,k} \geq \underline{Q}_{h,b}$ (per cell) | Minimum turbined flow (equipment limits)     | $\sigma^{q-}_{h,b,k}$ |
...
| $g_{h,b,k} \geq \underline{G}_{h,b}$ (per cell) | Minimum generation (grid services)           | $\sigma^{g-}_{h,b,k}$ |
...
- **Spillage cost**: Small regularization to prefer turbining over spilling
...
**Generation constraint** (depends on production model; evaluated per cell $(h,b)$ — see Unit Groups above):

- Constant productivity: $g_{h,b,k} = \rho_h \cdot q_{h,b,k}$
- FPHA: $g_{h,b,k} \leq \sigma_{h,b}\big(\gamma_0^m + \gamma_v^m \cdot v_h^{avg} + \gamma_s^m \cdot s_{h,k}\big) + \gamma_q^m \cdot q_{h,b,k}$ for each plane $m$
...
The curtailment cost is a small regularization term that incentivizes the solver to use all available generation before curtailing.
