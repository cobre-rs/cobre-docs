...
The exact deterministic upper bound and the `gap` stopping rule that compares it against the lower bound ([Stopping Rules §5](/math/stopping-rules)) are admissible only under the **expectation** risk measure: every stage must use expectation, or a CVaR measure with zero risk-aversion weight (expectation-equivalent). A CVaR measure with a positive risk-aversion weight $\lambda_t > 0$ at any stage is rejected at load with a named validation error rather than silently evaluating an unsound comparison — see [Upper Bound Evaluation §2](/math/upper-bound-evaluation) and [Stopping Rules §5](/math/stopping-rules) for the admissibility condition.

For risk-averse problems, the inner approximation (SIDP) remains the route to a deterministic upper bound: it provides deterministic upper bounds that remain valid regardless of the risk measure. See [Upper Bound Evaluation §1](/math/upper-bound-evaluation) for the complete formulation.
...
- [Stopping Rules](/math/stopping-rules) — Bound stalling recommended for risk-averse convergence monitoring; the gap rule is rejected at load under a risk-averse (CVaR, positive weight) measure
