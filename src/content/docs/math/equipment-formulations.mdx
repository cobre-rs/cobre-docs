...
The capacity limits $\bar{F}^+_l$ and $\bar{F}^-_l$ may vary by stage and, optionally, by block through absolute-MW overrides on `constraints/line_bounds.parquet` (`direct_mw`/`reverse_mw`, with an optional `block_id`). A line with no override rows keeps its base capacity unchanged.
:::
...
The dispatch LP is lossless: both flow directions enter the bus balances with coefficient ±1. Transmission losses are a reported quantity computed after the solve, $\text{loss}_{l,k} = (1 - \eta_l)\,(f^+_{l,k} + f^-_{l,k})$, where $\eta_l$ is the line's reported efficiency ([Notation Conventions](/overview/notation-conventions)). See [System Elements](/math/system-elements) for the network model.
