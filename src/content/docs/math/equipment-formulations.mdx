...
import { Content as EquipmentConfigure } from './_impl/_equipment.configure.mdx';
import { Content as EquipmentIO } from './_impl/_equipment.io.mdx';
import { Content as EquipmentNotes } from './_impl/_equipment.notes.mdx';
...
- $f^+_{\ell,k}$ = direct flow (source → target)
- $f^-_{\ell,k}$ = reverse flow (target → source)

**Bounds:**

$$
0 \leq f^+_{\ell,k} \leq \bar{F}^+_\ell, \quad 0 \leq f^-_{\ell,k} \leq \bar{F}^-_\ell
$$

:::note[Stage- and block-varying capacity]
The capacity limits $\bar{F}^+_\ell$ and $\bar{F}^-_\ell$ may vary by stage and, optionally, by block through absolute-MW overrides on `constraints/line_bounds.parquet` (`direct_mw`/`reverse_mw`, with an optional `block_id`). A line with no override rows keeps its base capacity unchanged.
:::
...
The dispatch LP is lossless: both flow directions enter the bus balances with coefficient ±1. Transmission losses are a reported quantity computed after the solve, $\text{loss}_{\ell,k} = (1 - \eta_\ell)\,(f^+_{\ell,k} + f^-_{\ell,k})$, where $\eta_\ell$ is the line's reported efficiency ([Notation Conventions](/overview/notation-conventions)). See [System Elements](/math/system-elements) for the network model.

**Objective Contribution:**

$$
\sum_{k} \tau_k \cdot c^{exch}_\ell (f^+_{\ell,k} + f^-_{\ell,k})
$$

:::note[Note on Exchange Cost]
The cost $c^{exch}_\ell$ is a **regularization term**, not an actual transmission cost. Its purpose is to:
...
Its value is set well below every generation and deficit cost, so that it only breaks ties; if it noticeably changes dispatch decisions, it is set too high.
...
<Tabs>
  <TabItem label="Configure"><EquipmentConfigure /></TabItem>
  <TabItem label="Inputs & Outputs"><EquipmentIO /></TabItem>
  <TabItem label="Implementation notes"><EquipmentNotes /></TabItem>
</Tabs>

## Cross-References

- [Notation conventions](/overview/notation-conventions) — variable and set definitions ($g_j$, $f_\ell$, $\chi_c$, $p_j$, $\tau_k$)
