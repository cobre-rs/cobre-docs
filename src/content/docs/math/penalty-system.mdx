...
| ------------------ | ------ | ------------------------- | ------------------------------- |
| `deficit_segments` | \$/MWh | Unmet load per bus        | Piecewise cost of load shedding |
| `excess_cost`      | \$/MWh | Excess generation per bus | Absorb uncontrollable surplus   |
...
The cost spans a wide range because it serves two distinct use cases. At the low end, it acts as a pure regularization term that breaks solver degeneracy without distorting the policy. At the high end, it serves a policy-shaping role, actively discouraging excess generation to keep dispatch solutions physically realistic. See the Configure tab below for the concrete case-level default.
:::
...
| Penalty                           | Units       | Applied To                     | Purpose                                            |
| --------------------------------- | ----------- | ------------------------------ | -------------------------------------------------- |
| `storage_violation_below_cost`    | \$/hm3      | Storage < min (dead volume)    | Reservoir below dead volume — near-physical limit  |
| `filling_target_violation_cost`   | \$/hm3      | Per-stage filling floor missed | Commissioning fill schedule — below `deficit`      |
| `turbined_violation_below_cost`   | \$/(m3/s·h) | Turbined flow < min            | Equipment limits / ecological flow                 |
| `outflow_violation_below_cost`    | \$/(m3/s·h) | Outflow < min                  | Environmental minimum flow (operator/regulatory)   |
| `outflow_violation_above_cost`    | \$/(m3/s·h) | Outflow > max                  | Downstream flooding prevention                     |
| `generation_violation_below_cost` | \$/MWh      | Generation < min               | Contractual or environmental minimum generation    |
| `evaporation_violation_cost`      | \$/(m3/s·h) | Evaporation constraint         | Physical constraint (bidirectional, see Section 5) |
| `water_withdrawal_violation_cost` | \$/(m3/s·h) | Unmet water withdrawal         | Human consumption / irrigation commitments         |
| `generic_violation_cost`          | varies      | Generic constraint violations  | User-defined physical or operational constraints   |
...
| Penalty            | Units       | Applied To                     | Purpose                                                                                                |
| ------------------ | ----------- | ------------------------------ | ------------------------------------------------------------------------------------------------------ |
| `spillage_cost`    | \$/(m3/s·h) | Water spilled                  | Prefer turbining over spilling when solver is indifferent                                              |
| `turbined_cost`    | \$/(m3/s·h) | Turbined flow (every hydro)    | Prevent interior FPHA solutions; must be > `spillage_cost` per plant (see below)                       |
| `diversion_cost`   | \$/(m3/s·h) | Water diverted                 | Prefer main channel flow; higher than spillage (water leaves cascade)                                  |
| `curtailment_cost` | \$/MWh      | Curtailed non-controllable gen | Prioritize using available non-controllable generation over curtailing it                              |
| `exchange_cost`    | \$/MWh      | Power flow on lines            | Prefer local supply; avoid unnecessary inter-bus power flows                                           |
...
During the filling period (`[start_stage_id, entry_stage_id)`), the hydro is in commissioning state: the reservoir accumulates water toward `min_storage_hm3` (dead volume) before it can begin generating. This is Cobre's own dead-volume filling model (`enchimento de volume morto`), whose per-stage accumulation schedule is specified below.

:::note[Before filling — the PreFilling phase]
These specifics govern the **filling** window `[start_stage_id, entry_stage_id)`. Before that window — and, for a **non-filling** hydro, at any stage outside its commissioning window — the plant is **PreFilling**: it does not yet exist in the dispatch. Turbine, spillage, and diversion are all pinned to zero, so **spillage is frozen and incurs no `spillage_cost`** (unlike during filling), storage is held constant by a frozen identity, and natural inflow is routed past the site to the first active downstream plant. See [system elements §5](/math/system-elements) for the full lifecycle.
:::
