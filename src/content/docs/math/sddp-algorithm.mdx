...
import { Content as SddpConfigure } from './_impl/_sddp.configure.mdx';
...
fwd: "1. Forward pass\nsample M trajectories → trial points, UB" {shape: rectangle}
bwd: "2. Backward pass\nevaluate N openings → 1 Benders cut / stage" {shape: rectangle}
conv: "3. Converged?\ngap = max(0, UB − LB) / max(1, |LB|) ≤ tol" {shape: diamond}
done: stopped {shape: oval}

fwd -> bwd: trial points
bwd -> conv: new cuts added
conv -> fwd: no · k ← k+1
conv -> done: yes
```
...
3. Record the optimal state $\hat{x}_t$ (end-of-stage storage volumes and updated AR lags), **canonicalized onto its admissible bounds** (see below), as the trial point for stage $t$
...
**State canonicalization**: The recorded outgoing state $\hat{x}_t$ is not the raw LP primal but its projection onto the resolved admissible bounds. Each bounded coordinate — end-of-stage storage, in-transit water volume, and any anticipated-thermal commitment hold — is clamped into its resolved interval, while the unbounded inflow (AR) lags pass through unchanged. The projection is the identity for an in-box optimum, so a solve whose state stays within bounds is unaffected by it. It is applied **once**, at read-back, and every consumer reads that one canonical vector: the forward trajectory, the backward-pass trial state $\hat{x}_{t-1}$, and the cut intercept anchored at it (see [Cut Management](/math/cut-management)). This is what makes $\hat{x}_t$ a well-defined function of the stage's optimal value and its resolved bounds — independent of which degenerate optimal vertex the solve terminated at or of round-off. See [Determinism Guarantees](/math/determinism-guarantees#3-methodology-mechanisms).
...
"Every scenario $\omega \in \Omega_t$" refers to all $N_{\text{openings}}$ noise vectors in the **fixed opening tree** for stage $t$. This is the **Complete** backward sampling scheme — the backward pass evaluates ALL openings (the same set across all iterations), regardless of the forward pass noise source. An alternative variant would sample $n$ openings instead of the full tree; Cobre uses the Complete scheme because it guarantees that cut coefficients average over the full scenario distribution at each stage. The aggregation probabilities $p(\omega)$ in [Cut Management](/math/cut-management) are uniform over these openings ($p(\omega) = 1/N_{\text{openings}}$). See [Scenario Generation](/math/scenario-generation).
:::
...
classes: {
  st: {shape: circle; width: 56; height: 56}
  op: {shape: circle; width: 50; height: 50}
}

forward: "Forward pass — sparse (sample M paths)" {
  direction: right
...
backward: "Backward pass — exhaustive (all N openings)" {
  direction: down

  trial: "trial point x̂"
  o1: "ξ₁" {class: op}
  o2: "ξ₂" {class: op}
  o3: "ξ₃" {class: op}
  agg: "aggregate\nπ̄ = E[π],  ᾱ = E[α]"
  cut: "one Benders cut\nadded to stage t−1" {style.stroke-dash: 4}
...
forward -> backward: "each visited trial point x̂"
```

### 3.3 Convergence Monitoring

**Lower Bound**: The risk-adjusted lower bound is computed by solving the stage-1 LP for every opening in the stage-1 scenario set, collecting the per-opening objectives, and aggregating them through the stage-1 risk measure (see [Stage Indexing](/overview/notation-conventions#stage-indexing) for the 1-based convention used throughout this spec):

$$
\underline{z}^k = \rho_1 \Big[ Q_1^k(x_0, \omega) \;\Big|\; \omega \in \Omega_1 \Big]
$$

where $Q_1^k(x_0, \omega)$ is the optimal objective of the stage-1 LP under opening $\omega$ with the current cut approximation at iteration $k$, $x_0$ is the known initial state, and $\rho_1$ is the stage-1 risk measure (expected value under risk-neutral, or CVaR under risk-averse). The per-opening probabilities are uniform: $p(\omega) = 1 / |\Omega_1|$.

Under risk-neutral settings ($\rho_1 = \mathbb{E}$), this reduces to a simple average over all stage-1 opening objectives. Because cuts are added monotonically and never removed within a training run, this bound is non-decreasing across iterations — see [Cut Management](/math/cut-management) for the append-only guarantee.

**Upper Bound**: Under a sampled forward pass, this is a **statistical** estimate — the sample mean over the forward pass trajectories; under an enumerated forward pass, it is the **exact** probability-weighted bound $\sum_\ell P(\ell)\, C(\ell)$ over every leaf path, with no sampling error (see [Upper Bound Evaluation](/math/upper-bound-evaluation)). The sampled-mean form:

$$
\bar{z}^k = \frac{1}{M} \sum_{m=1}^{M} \sum_{t=1}^{T} c_t^\top x_t^{(m)}
$$

**Optimality Gap**: the difference between the two bounds, clamped to stay non-negative under sampling or floating-point noise:

$$
\text{gap}^k = \frac{\max\bigl(0,\; \bar{z}^k - \underline{z}^k\bigr)}{\max(1, |\underline{z}^k|)}
$$

See [Upper Bound Evaluation](/math/upper-bound-evaluation), section 9, for the exact-mechanism gap this overview simplifies, and [Stopping Rules](/math/stopping-rules), section 5, for how the gap-based stopping rule consumes it.
...
**Reduced-cost cut extraction**: Each state variable (incoming storage, inflow lag, in-transit water bucket, or anticipated-thermal slot) is pinned by column bounds, and the reduced cost of that pinned column gives the cut coefficient directly — no preprocessing or dual combination is needed. FPHA hyperplane and generic constraint effects are captured automatically by the LP solver through the pinned column's reduced cost. See [Cut Management](/math/cut-management).

The software architecture that realises these constraints is described in the [`cobre-sddp` README](https://github.com/cobre-rs/cobre/blob/main/crates/cobre-sddp/README.md).

## 4. Policy Graph Structure

The stage chain used throughout this spec — one node per stage $t \in \{1, \ldots, T\}$, connected by transitions with probability $p = 1$, terminating at $V_{T+1}(x) = 0$ — is the special case of a more general **policy graph**: a structure of nodes, probability-weighted transitions, and per-node future-cost pools. See [Policy Graphs](/math/policy-graphs) for the full node/transition/pool structure, the two probability axes (between-node edge weights versus within-node openings), the discount-separate convention, and the mapping onto the SDDP.jl family.

Cobre's policy graph is **finite-horizon only**: an acyclic, leaf-terminated graph in which every trajectory starts at a root and reaches a leaf after a bounded number of stages. A **cyclic** graph shape — a later stage transitioning back to an earlier one, for infinite-periodic-horizon planning — is **reserved** and rejected at setup rather than a currently accepted mode. See [Horizon Modes](/math/horizon-modes) for the cyclic target design this reservation anticipates, and [Policy Graphs](/math/policy-graphs) for the complete list of structural limitations.
...
| In-transit water buckets  | $b_{i,d}$              | $B$          | Released water still in transit under travel-time delay — one bucket per receiving plant $i$ per maturity lag $d$ |
| Anticipated-thermal slots | $y^{i}_{k}$            | $\sum_i K_i$ | Pending anticipated-thermal commitments awaiting delivery — one ring of $K_i$ slots per anticipated plant |

The last two components are present only when the study declares them: water
travel-time arcs introduce the in-transit buckets, and anticipated thermals
introduce the ring slots. A study with neither reduces to the classical
storage (+ AR-lag) state. All four components are genuine Markov state — each is
pinned by column bounds on its incoming-state column, and the reduced cost of
that column is its cut coefficient (section 3.2).
...
Two further mechanisms expand the state vector beyond storage and AR lags, and
both are carried as Markov state by the same column-bound pinning as the AR
lags — their pinned columns' reduced costs enter the cut coefficients directly.

**In-transit water buckets.** When a hydro's main cascade arc carries a
`travel_time_hours`, released water reaches its downstream plant only after a
delay. The volume in transit is tracked as augmented Benders state — one bucket
slot per downstream plant per maturity lag — so that a stage's future cost
depends only on the current in-transit volumes, not on the release path that
produced them.

**Anticipated-thermal slots.** An anticipated thermal commits generation $K_i$
stages ahead of delivery. Each pending commitment occupies a slot in a per-plant
ring of length $K_i$; the ring shifts one position per stage until the
commitment matures at its delivery stage. The ring slots are Markov state so
that the cut at each stage prices the cost of commitments already in flight.

Both expansions are optional and instance-specific; see
[System Elements](/math/system-elements) for the entity configuration and
[LP Formulation](/math/lp-formulation) for the in-transit bucket rows (§5d) and
anticipated-thermal delivery gating (§5c) in the stage LP.
...
The methodology above defines the forward/backward pass structure and the execution-model constraints that shape it; the tab below covers how Cobre's software surface configures backward-pass work scheduling.

<Tabs>
  <TabItem label="Configure"><SddpConfigure /></TabItem>
</Tabs>
...
- [Policy Graphs](/math/policy-graphs) — The node/transition/pool generalisation of the stage chain; two probability axes, discount-separate convention, and structural limitations (finite-horizon only, cyclic reserved)
...
- [Determinism Guarantees](/math/determinism-guarantees) — Read-back canonicalization of the trial-point values onto admissible bounds, and the reproducibility mechanisms (canonical state-block ordering, pinned solve order) it rests on
...
- [Horizon Modes](/math/horizon-modes) — The reserved cyclic policy-graph target design (season function, cycle convergence inequality, season-indexed cut pool, fixed-point Bellman operator) that the finite-horizon-only restriction in [Policy Graphs](/math/policy-graphs) anticipates
...
- **Running Cobre:** [Running Studies](/running/running-studies/) — the software workflow that trains and runs this algorithm.
