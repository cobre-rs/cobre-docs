...
For the storage fixing constraint (see [LP Formulation §4a](../specs/math/lp-formulation.md)):
...
The hot-path solver update pattern (modifying RHS via `changeRowBounds` for incoming state, extracting duals via `getRowDual` for cut coefficients) is documented in the [Solver HiGHS Implementation](../specs/architecture/solver-highs-impl.md) and [Solver Abstraction](../specs/architecture/solver-abstraction.md) specs. The key property: since incoming state variables appear on the RHS of fixing constraints with coefficient $+1$, no sign change is needed when mapping duals to cut coefficients ($\pi^v_h = \pi^{fix}_h$; for AR lags, $\pi^{lag}_{h,\ell}$ is used directly). Cut coefficient extraction is a single contiguous slice read: `dual[0..n_state]`.
...
- [Theory: SDDP Theory](../theory/sddp-theory.md) — Algorithm overview and cut generation process
- [Theory: Cut Management](../theory/cut-management.md) — Cut coefficient computation and aggregation details
- [Theory: PAR(p) Autoregressive Models](../theory/par-model.md) — Detailed PAR(p) model using inflow parameters defined here
- [Specs: LP Formulation](../specs/math/lp-formulation.md) — Complete LP subproblem using this notation
- [Specs: Cut Management](../specs/math/cut-management.md) — Formal cut management specification
- [Specs: PAR Inflow Model](../specs/math/par-inflow-model.md) — PAR(p) inflow model specification
- [Specs: Hydro Production Models](../specs/math/hydro-production-models.md) — FPHA plane coefficients and productivity
- [Specs: Equipment Formulations](../specs/math/equipment-formulations.md) — Thermal, contract, pumping variable notation
