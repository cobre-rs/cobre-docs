...
| [`cobre-solver`](./solver.md) | <span class="status-experimental">experimental</span> | LP/MIP solver abstraction with HiGHS backend |
...
**Solvers are pluggable.** `cobre-solver` provides a trait-based abstraction. The default backend is HiGHS (open-source), but commercial solvers like Gurobi can be added behind the same interface.
