...
1. Solve the stage LP once per opening $\omega \in \Omega_t$, using $\hat{x}_{t-1}$ as the incoming state and $\omega$ as the realization of all uncertain quantities (inflows and, if applicable, load demand).
...
- [Stochastic Modeling](stochastic-modeling.md) -- Background on inflow uncertainty, PAR(p) models, and load seasonal statistics
