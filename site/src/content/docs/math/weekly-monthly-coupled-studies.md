---
title: Weekly+Monthly Coupled Studies
description: Two-study coupling architecture — terminal boundary cut import, recent-observation lag seeding, and independent per-stage scenario counts for weekly runs informed by upstream monthly policy.
---
...
[Multi-Resolution Studies](/math/multi-resolution-studies), which addresses
...
supports. See [Scenario Generation](/math/scenario-generation) for the
...
[Horizon Modes](/math/horizon-modes).

## Cross-References

- [PAR Inflow Model](/math/par-inflow-model) — AR lag accumulation; seasonal parameters; model order selection; the lag buffer that is seeded from recent observations at stage 0 of the weekly run
