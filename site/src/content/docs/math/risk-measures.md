...
Source figure (matplotlib `d22-risk-measure-cvar.svg`, to be rebuilt as a theme-adaptive D2/Observable Plot figure in E4): cost distribution with E[C] and CVaR*α marked, the tail region shaded, and the convex combination ρ = λ·CVaR*α + (1−λ)·E[C].
:::
...
This sorting procedure produces the same $\mu^*$ as solving the dual LP from §4.2, because the LP maximizes a linear objective with per-scenario upper bounds — a structure whose optimal solution is the greedy allocation above.
:::
...
For risk-averse problems, the inner approximation (SIDP) provides deterministic upper bounds that remain valid regardless of the risk measure. See [Upper Bound Evaluation §1](/math/upper-bound-evaluation) for the complete formulation.

## 10 Lower Bound Validity with Risk Measures

:::caution[Critical Warning]
The lower bound computed during SDDP training is **NOT a valid bound** for risk-averse problems.
:::
...
| Purpose                    | Method                                                                                                    |
| -------------------------- | --------------------------------------------------------------------------------------------------------- |
| **Convergence monitoring** | Track $\underline{z}$ stabilization (bound stalling rule — see [Stopping Rules §4](/math/stopping-rules)) |
| **Valid upper bound**      | Inner approximation (SIDP) — see [Upper Bound Evaluation](/math/upper-bound-evaluation)                   |
| **Policy evaluation**      | Monte Carlo simulation with risk-averse policy decisions                                                  |

:::note[Convergence reporting]
When risk measures are enabled, convergence reports should label the "lower bound" as "convergence indicator" or explicitly note that it is not a valid bound.
:::
...
- [Stopping Rules](/math/stopping-rules) — Bound stalling recommended for risk-averse convergence monitoring; simulation-based stopping limitations
- [Discount Rate](/math/discount-rate) — Discount factor $d$ convention and discounted Bellman equation
- [Horizon Modes](/math/horizon-modes) — Interaction of risk measures with cyclic policy graphs
- [Upper Bound Evaluation](/math/upper-bound-evaluation) — SIDP inner approximation for valid upper bounds with CVaR objectives
