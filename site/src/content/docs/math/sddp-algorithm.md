...
Source figure (`d02-value-function.svg`): Value function approximation via Benders cuts — each iteration adds a cut at a new trial point, tightening the outer approximation toward the true cost-to-go function.
:::
...
:::note[Figure — retooled in E4]
Source figure (`d03-scenario-tree.svg`): Scenario tree — forward pass samples M sparse paths while backward pass evaluates all N openings at each trial point.
:::
