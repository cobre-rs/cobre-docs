...
`astro-d2` renders the conceptual SDDP-loop diagram below to inline SVG with the
ELK layout. It is a small but representative 4-node flow — the kind of shape and
edge styling E4 will reuse for real figures. The `.d2-svg` keystone CSS re-keys
d2's palette classes (`fill-N*` / `stroke-*`) to `:root[data-theme]`, so the
figure flips with the manual theme toggle rather than only the OS preference:

```d2
direction: right

forward: Forward pass
backward: Backward pass {shape: hexagon}
cuts: Cut pool
policy: Policy {style.stroke-dash: 4}

forward -> backward: scenario trajectories
backward -> cuts: add Benders cuts
cuts -> forward: tighten value function
backward -> policy: converged
```
