...
- **KaTeX**: manual remark-math + rehype-katex (D4) wired in `astro.config.mjs`
  ...
  client JS).
- **Source**: all chapters live under `src/content/docs/` (the repo-root path
  after promotion from `site/src/content/docs/` at decommission time); the
  sidebar/TOC is configured in `astro.config.mjs` (Starlight `sidebar`), not a
  flat text table-of-contents file.
